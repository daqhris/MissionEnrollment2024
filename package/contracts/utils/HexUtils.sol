...
library HexUtils {
   ...
   */
  function hexStringToBytes32(bytes memory str, uint256 idx, uint256 lastIdx) internal pure returns (bytes32, bool) {
  ...
  function addressToHex(address addr) internal pure returns (string memory) {
    ...
    for (uint i = 0; i < 20; i++) {
      ...
      bytes1 lowNibble = bytes1(uint8(byteValue) - 16 * uint8(highNibble));
  ...
  function _nibbleToHexChar(bytes1 nibble) internal pure returns (bytes1 hexChar) {
