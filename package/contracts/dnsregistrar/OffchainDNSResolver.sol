...
contract OffchainDNSResolver is IExtendedResolver, IERC165 {
  ...
  function resolveCallback(bytes calldata response, bytes calldata extraData) external view returns (bytes memory) {
    (bytes memory name, bytes memory query, bytes4 selector) = abi.decode(extraData, (bytes, bytes, bytes4));

    if (selector != bytes4(0)) {
      (bytes memory targetData, address targetResolver) = abi.decode(query, (bytes, address));
      return
        callWithOffchainLookupPropagation(
          targetResolver,
          name,
          query,
          abi.encodeWithSelector(selector, response, abi.encode(targetData, address(this)))
        );
    }
        ...
        if (IERC165(dnsresolver).supportsInterface(IExtendedDNSResolver.resolve.selector)) {
          ...
          return
            callWithOffchainLookupPropagation(
              ...
              name,
              query,
              abi.encodeCall(IExtendedResolver.resolve, (name, query))
            );
        } else {
  ...
  function callWithOffchainLookupPropagation(
    ...
    bool result = LowLevelCallUtils.functionStaticCall(address(target), data);
  ...
  function revertWithDefaultOffchainLookup(bytes memory name, bytes memory data) internal view {
  ...
  function handleOffchainLookupError(bytes memory returnData, address target, bytes memory name) internal view {
    (
      address sender,
      string[] memory urls,
