...
contract DNSRegistrar is IDNSRegistrar, IERC165 {
  ...
  constructor(address _previousRegistrar, address _resolver, DNSSEC _dnssec, PublicSuffixList _suffixes, ENS _ens) {
  ...
  function supportsInterface(bytes4 interfaceID) external pure override returns (bool) {
    return interfaceID == type(IERC165).interfaceId || interfaceID == type(IDNSRegistrar).interfaceId;
  }
  ...
  function _enableNode(bytes memory domain, uint256 offset) internal returns (bytes32 node) {
