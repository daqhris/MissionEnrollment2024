...
task("seed", "Creates test subbdomains and wraps them with Namewrapper")
  ...
  .setAction(async ({ name }, hre) => {
    ...
    console.log("Account balance:", publicClient.getBalance({ address: deployer.account.address }));
    console.log({
      registryAddress,
      registrarAddress,
      wrapperAddress,
    ...
    const EnsRegistry = await hre.viem.getContractAt("ENSRegistry", registryAddress);
    const BaseRegistrar = await hre.viem.getContractAt("BaseRegistrarImplementation", registrarAddress);
    const NameWrapper = await hre.viem.getContractAt("NameWrapper", wrapperAddress);
    const Resolver = await hre.viem.getContractAt("PublicResolver", resolverAddress);
    ...
    await NameWrapper.write.wrapETH2LD([name, firstAddress, CAN_DO_EVERYTHING, resolverAddress], {
      gas: 10000000n,
    });
    console.log(`Wrapped NFT for ${domain} is available at ${getOpenSeaUrl(NameWrapper.address, namehashedname)}`);
    await NameWrapper.write.setSubnodeOwner([namehash(`${name}.eth`), "sub1", firstAddress, CAN_DO_EVERYTHING, 0n]);
    console.log("NameWrapper setSubnodeOwner successful for sub1");
    await NameWrapper.write.setSubnodeOwner([namehash(`${name}.eth`), "sub2", firstAddress, CAN_DO_EVERYTHING, 0n]);
    console.log("NameWrapper setSubnodeOwner successful for sub2");
    await NameWrapper.write.setResolver([namehash(`sub2.${name}.eth`), resolverAddress]);
    console.log("NameWrapper setResolver successful for sub2");
    await Resolver.write.setText([namehash(`sub2.${name}.eth`), "domains.ens.nft.image", ""]);
    await Resolver.write.setText([namehash(`sub2.${name}.eth`), "avatar", "https://i.imgur.com/1JbxP0P.png"]);
    console.log(
    ...
    await NameWrapper.write.setFuses([namehash(`sub2.${name}.eth`), CANNOT_UNWRAP], {
      gas: 10000000n,
    });
    console.log("NameWrapper set CANNOT_UNWRAP fuse successful for sub2");
    await NameWrapper.write.setFuses([namehash(`sub2.${name}.eth`), CANNOT_SET_RESOLVER], {
      gas: 10000000n,
    });
    console.log("NameWrapper set CANNOT_SET_RESOLVER fuse successful for sub2");
    await NameWrapper.write.unwrap([namehash(`${name}.eth`), labelhash("sub1"), firstAddress], {
      gas: 10000000n,
    });
