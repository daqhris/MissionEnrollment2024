...
const WIKI_DEPLOYMENTS_URL = 'https://raw.githubusercontent.com/wiki/ensdomains/ens-contracts/ENS-Contract-Deployments.md';
const getChainDeploymentsFromWiki = ({ chainIndex, lines, }) => {
    ...
    const indexOfNextChain = lines.findIndex((line, index) => index > indexOfChain && line.includes('#'));
    ...
    const chainDeployments = lines.slice(startOfChainDeployments, indexOfNextChain);
    return chainDeployments;
};
const checkDeployment = async ({ chainName, deploymentFilenames, wikiDeployments, deploymentIndex, }) => {
    ...
    const wikiDeploymentString = wikiDeployments.find((wikiDeployment) => {
        ...
        const match = wikiDeploymentName.match(new RegExp(`${deploymentFilename.split('.')[0].trim()}`));
        return match && match?.[0] === match?.input;
    });
    if (!wikiDeploymentString)
        throw new Error(`Deployment ${deploymentIndex} not found in wiki for ${chainName}`);
    const wikiDeploymentAddress = wikiDeploymentString.substring(wikiDeploymentString.indexOf('[') + 1, wikiDeploymentString.lastIndexOf(']'));
    const wikiEtherscanAddress = wikiDeploymentString.substring(wikiDeploymentString.lastIndexOf('/') + 1, wikiDeploymentString.lastIndexOf(')'));
    const deployment = await import(`../deployments/${chainName}/${deploymentFilename}`);
    if (deployment.address !== wikiDeploymentAddress) {
        throw new Error(`Deployment ${deploymentIndex} in wiki and in the repository do not match for ${chainName}. Wiki: ${wikiDeploymentAddress}, Deployment: ${deployment.address}`);
    }
    if (deployment.address !== wikiEtherscanAddress) {
        throw new Error(`Etherscan address ${deploymentIndex} in wiki and in the repository do not match for ${chainName}. Wiki Etherscan: ${wikiEtherscanAddress}, Deployment: ${deployment.address}`);
    }
};
const checkChain = async ({ chainIndex, lines }) => {
    ...
    if (wikiDeployments.length !== deploymentFilenames.length) {
        throw new Error(`Number of deployments in wiki and in the repository do not match for ${SUPPORTED_CHAINS[chainIndex]}`);
    }
