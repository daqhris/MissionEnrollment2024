import { expiration, hexEncodeSignedSet, inception, rootKeys, testRrset } from "../fixtures/dns.js";
...
async function fixture() {
  ...
  await suffixes.write.addPublicSuffixes([[dnsEncodeName("test"), dnsEncodeName("co.nz")]]);
...
describe("DNSRegistrar", () => {
  it("sets constructor variables correctly", async () => {
    ...
    await expect(dnsRegistrar.read.oracle()).resolves.toEqualAddress(dnssec.address);
    await expect(dnsRegistrar.read.ens()).resolves.toEqualAddress(ensRegistry.address);
  });
  it("allows anyone to claim on behalf of the owner of an ENS name", async () => {
    ...
    const proof = [
      ...
      hexEncodeSignedSet(testRrset({ name: "foo.test", address: accounts[0].address })),
    ];
    await dnsRegistrar.write.proveAndClaim([dnsEncodeName("foo.test"), proof], {
      account: accounts[1],
    });
    await expect(ensRegistry.read.owner([namehash("foo.test")])).resolves.toEqualAddress(accounts[0].address);
  });
  it("allows claims on names that are not TLDs", async () => {
    const { dnsRegistrar, ensRegistry, accounts } = await loadFixture(fixture);
    const proof = [
      hexEncodeSignedSet(rootKeys({ expiration, inception })),
      hexEncodeSignedSet(testRrset({ name: "foo.co.nz", address: accounts[0].address })),
    ];
    await dnsRegistrar.write.proveAndClaim([dnsEncodeName("foo.co.nz"), proof]);
    await expect(ensRegistry.read.owner([namehash("foo.co.nz")])).resolves.toEqualAddress(accounts[0].address);
  });
  it("allows anyone to update a DNSSEC referenced name", async () => {
    ...
    await dnsRegistrar.write.proveAndClaim([dnsEncodeName("foo.test"), proof]);
    await expect(ensRegistry.read.owner([namehash("foo.test")])).resolves.toEqualAddress(accounts[0].address);
    const newProof = [proof[0], hexEncodeSignedSet(testRrset({ name: "foo.test", address: accounts[1].address }))];
    await dnsRegistrar.write.proveAndClaim([dnsEncodeName("foo.test"), newProof]);
    await expect(ensRegistry.read.owner([namehash("foo.test")])).resolves.toEqualAddress(accounts[1].address);
  });
  ...
  it("does not allow updates with stale records", async () => {
    ...
    const tx = dnsRegistrar.write.proveAndClaim([dnsEncodeName("foo.test"), newProof]);
    await expect(dnssec).transaction(tx).toBeRevertedWithCustomError("SignatureExpired");
  });
  it("allows the owner to claim and set a resolver", async () => {
    ...
    await expect(ensRegistry.read.owner([namehash("foo.test")])).resolves.toEqualAddress(accounts[0].address);
    await expect(ensRegistry.read.resolver([namehash("foo.test")])).resolves.toEqualAddress(accounts[1].address);
  });
  it("does not allow anyone else to claim and set a resolver", async () => {
    const { dnsRegistrar, accounts } = await loadFixture(fixture);
    const proof = [
      hexEncodeSignedSet(rootKeys({ expiration, inception })),
      hexEncodeSignedSet(testRrset({ name: "foo.test", address: accounts[1].address })),
    ];
    await expect(dnsRegistrar)
      .write("proveAndClaimWithResolver", [dnsEncodeName("foo.test"), proof, accounts[1].address, zeroAddress])
  ...
  it("sets an address on the resolver if provided", async () => {
    ...
    await expect(publicResolver.read.addr([namehash("foo.test")])).resolves.toEqualAddress(accounts[0].address);
  });
  it("forbids setting an address if the resolver is not also set", async () => {
    ...
    await expect(dnsRegistrar)
      .write("proveAndClaimWithResolver", [dnsEncodeName("foo.test"), proof, zeroAddress, accounts[0].address])
  ...
  describe("unrelated proof", () => {
    ...
    it("cannot claim multiple names using single unrelated proof", async () => {
      const { ensRegistry, dnsRegistrar, accounts } = await loadFixture(fixtureWithTestTld);
      const alice = accounts[1];
      // Build sample proof for a DNS record with name `alice.test` that alice owns
      const proofForAliceDotTest = [
        hexEncodeSignedSet(rootKeys({ expiration, inception })),
        hexEncodeSignedSet(testRrset({ name: "alice.test", address: alice.address })),
      ];
      // This is the expected use case.
      // Using the proof for `alice.test`, can claim `alice.test`
      await dnsRegistrar.write.proveAndClaim([dnsEncodeName("alice.test"), proofForAliceDotTest]);
      await expect(ensRegistry.read.owner([namehash("alice.test")])).resolves.toEqualAddress(alice.address);
      // Now using the same proof for `alice.test`, alice cannot also claim `foo.test`
      await expect(dnsRegistrar)
        .write("proveAndClaim", [dnsEncodeName("foo.test"), proofForAliceDotTest])
        .toBeRevertedWithCustomError("NoOwnerRecordFound");
    });
    it("cannot takeover claimed DNS domains using unrelated proof", async () => {
      ...
      const proofForBobDotTest = [
        hexEncodeSignedSet(rootKeys({ expiration, inception })),
        hexEncodeSignedSet(testRrset({ name: "bob.test", address: bob.address })),
      ];
      // Bob cannot claim alice's domain
      await expect(dnsRegistrar)
        .write("proveAndClaim", [dnsEncodeName("alice.test"), proofForBobDotTest])
        .toBeRevertedWithCustomError("NoOwnerRecordFound");
    });
  });
});
