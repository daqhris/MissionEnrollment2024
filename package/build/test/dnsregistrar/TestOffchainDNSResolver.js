...
import { encodeAbiParameters, encodeFunctionData, getAddress, labelhash, namehash, parseAbiParameters, toFunctionSelector, zeroAddress, zeroHash, } from 'viem';
import { expiration, hexEncodeSignedSet, inception, rootKeys, rrsetWithTexts, } from '../fixtures/dns.js';
...
async function fixture() {
    ...
    const offchainResolver = await hre.viem.deployContract('MockOffchainResolver', []);
    const offchainDnsResolver = await hre.viem.deployContract('OffchainDNSResolver', [ensRegistry.address, dnssec.address, OFFCHAIN_GATEWAY]);
    const ownedResolver = await hre.viem.deployContract('OwnedResolver', []);
    const dummyResolver = await hre.viem.deployContract('DummyNonCCIPAwareResolver', [offchainDnsResolver.address]);
    ...
    const dnsGatewayAbi = await hre.artifacts
        ...
        .then((a) => a.abi);
    const doDnsResolveCallback = async ({ name, texts, calldata, }) => {
        const proof = [
    ...
    const doResolveCallback = async ({ extraData, result, }) => {
        const validUntil = BigInt(Math.floor(Date.now() / 1000 + 10000));
        const response = encodeAbiParameters([{ type: 'bytes' }, { type: 'uint64' }, { type: 'bytes' }], [result, validUntil, '0x']);
...
describe('OffchainDNSResolver', () => {
    ...
    it('handles calls to resolveCallback() with valid DNS TXT records containing an address', async () => {
        ...
        await expect(doDnsResolveCallback({
            name,
