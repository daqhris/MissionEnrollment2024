...
export async function deployNameWrapperFixture() {
  ...
  const nameWrapperUpgraded = await hre.viem.deployContract("UpgradedNameWrapperMock", [
    ensRegistry.address,
    baseRegistrar.address,
  ]);
  // setup .eth
  ...
  // setup .xyz
  await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("xyz"), accounts[0].address]);
  return {
    ensRegistry,
    baseRegistrar,
    metadataService,
