...
export const setSubnodeRecordTests = () =>
  describe("setSubnodeRecord()", () => {
    ...
    it("Can be called by the owner of a name", async () => {
      const { ensRegistry, nameWrapper, actions, accounts, resolverAddress } = await loadFixture(
    ...
    it("Transfers the wrapped token to the target address.", async () => {
      const { nameWrapper, actions, accounts, resolverAddress } = await loadFixture(setSubnodeRecordFixture);
      await actions.setSubnodeRecord.onNameWrapper({
        parentName: name,
        label: sublabel,
    ...
    it("Will not allow wrapping with a target address of 0x0", async () => {
      const { nameWrapper, resolverAddress } = await loadFixture(setSubnodeRecordFixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, zeroAddress, resolverAddress, 0n, 0, 0n])
    ...
    it("Will not allow wrapping with a target address of the wrapper contract address.", async () => {
      const { nameWrapper, resolverAddress } = await loadFixture(setSubnodeRecordFixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, nameWrapper.address, resolverAddress, 0n, 0, 0n])
    ...
    it("Does not allow anyone else to wrap a name even if the owner has authorised the wrapper with the ENS registry.", async () => {
      const { ensRegistry, nameWrapper, accounts, resolverAddress } = await loadFixture(setSubnodeRecordFixture);
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      await ensRegistry.write.setApprovalForAll([accounts[1].address, true]);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, accounts[0].address, resolverAddress, 0n, 0, 0n], {
          account: accounts[1],
        })
    ...
    it("Allows fuses to be burned if not expired and PARENT_CANNOT_CONTROL/CANNOT_UNWRAP are burned", async () => {
      ...
      const [, fuses] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(fuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CANNOT_TRANSFER);
    });
    ...
    it("Emits Wrap event", async () => {
      const { nameWrapper, accounts } = await loadFixture(setSubnodeRecordFixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, accounts[1].address, accounts[0].address, 0n, 0, 0n])
    ...
    it("Sets the appropriate values on the ENS registry", async () => {
      const { ensRegistry, nameWrapper, actions, accounts } = await loadFixture(setSubnodeRecordFixture);
      await actions.setSubnodeRecord.onNameWrapper({
        parentName: name,
        label: sublabel,
        owner: accounts[1].address,
    ...
    it("Will not create a subdomain with an empty label", async () => {
      const { nameWrapper, resolverAddress } = await loadFixture(setSubnodeRecordFixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), "", zeroAddress, resolverAddress, 0n, 0, 0n])
    ...
    it("Unwrapping a previously wrapped unexpired name retains PCC and so reverts setSubnodeRecord", async () => {
      const { ensRegistry, nameWrapper, actions, accounts, baseRegistrar } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      const [ownerBefore, fusesBefore, expiryBefore] = await nameWrapper.read.getData([toNameId(subname)]);
      ...
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, accounts[1].address, zeroAddress, 0n, 0, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
    it("Rewrapping a name that had PCC burned, but has now expired is possible", async () => {
      const { nameWrapper, actions, accounts, baseRegistrar, testClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
