...
export const setFusesTests = () => {
  describe("setFuses()", () => {
    ...
    it("Errors when manually changing calldata to incorrect type", async () => {
      ...
      data = data.substring(0, data.length - rogueFuse.length);
      data += rogueFuse;
    ...
    it("Will not allow burning fuses if PARENT_CANNOT_CONTROL has not been burned", async () => {
      ...
      await expect(nameWrapper)
        .write("setFuses", [namehash(`sub.${name}`), CANNOT_UNWRAP | CANNOT_TRANSFER])
    ...
    it("Can be called by the owner", async () => {
      ...
      expect(newFuses).toEqual(CANNOT_UNWRAP | CANNOT_TRANSFER | PARENT_CANNOT_CONTROL | IS_DOT_ETH);
    });
    it("Emits FusesSet event", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(namehash(name), CANNOT_UNWRAP | CANNOT_TRANSFER | PARENT_CANNOT_CONTROL | IS_DOT_ETH);
      const [, fuses, expiry] = await nameWrapper.read.getData([toNameId(name)]);
      expect(fuses).toEqual(CANNOT_UNWRAP | CANNOT_TRANSFER | PARENT_CANNOT_CONTROL | IS_DOT_ETH);
    ...
    it("Returns the correct fuses", async () => {
      ...
      const { result: fusesReturned } = await nameWrapper.simulate.setFuses([namehash(name), CANNOT_TRANSFER]);
      expect(fusesReturned).toEqual(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | IS_DOT_ETH);
    });
    ...
    it("Allows burning unknown fuses", async () => {
      ...
      const [, fuses] = await nameWrapper.read.getData([toNameId(name)]);
      expect(fuses).toEqual(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | IS_DOT_ETH | 64);
    });
    it("Logically ORs passed in fuses with already-burned fuses.", async () => {
      ...
      const [, fuses] = await nameWrapper.read.getData([toNameId(name)]);
      expect(fuses).toEqual(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | IS_DOT_ETH | 64 | CANNOT_TRANSFER);
    });
    it("can set fuses and then burn ability to burn fuses", async () => {
      ...
      await expect(nameWrapper.read.allFusesBurned([namehash(name), CANNOT_BURN_FUSES])).resolves.toEqual(true);
    ...
    it("can set fuses and burn transfer", async () => {
      ...
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      // check flag in the wrapper
      await expect(nameWrapper.read.allFusesBurned([namehash(name), CANNOT_TRANSFER])).resolves.toEqual(true);
    ...
    it("can set fuses and burn canSetResolver and canSetTTL", async () => {
      ...
      await nameWrapper.write.setFuses([namehash(name), CANNOT_SET_RESOLVER | CANNOT_SET_TTL]);
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      // check flag in the wrapper
      await expect(
        nameWrapper.read.allFusesBurned([namehash(name), CANNOT_SET_RESOLVER | CANNOT_SET_TTL]),
    ...
    it("can set fuses and burn canCreateSubdomains", async () => {
      const { ensRegistry, nameWrapper, actions, accounts } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      await expect(nameWrapper.read.allFusesBurned([namehash(name), CANNOT_CREATE_SUBDOMAIN])).resolves.toEqual(false);
      ...
      await nameWrapper.write.setFuses([namehash(name), CAN_DO_EVERYTHING | CANNOT_CREATE_SUBDOMAIN]);
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      await expect(nameWrapper.read.allFusesBurned([namehash(name), CANNOT_CREATE_SUBDOMAIN])).resolves.toEqual(true);
      // try to create a subdomain
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), "uncreatable", accounts[0].address, 0, 86400n])
        .toBeRevertedWithCustomError("OperationProhibited")
