...
export const approveTests = () => {
  describe("approve()", () => {
    ...
    it("Sets an approval address if owner", async () => {
      ...
      await expect(nameWrapper.read.getApproved([toNameId(name)])).resolves.toEqualAddress(accounts[1].address);
    });
    it("Sets an approval address if is an operator", async () => {
      ...
      await expect(nameWrapper.read.getApproved([toNameId(name)])).resolves.toEqualAddress(accounts[2].address);
    });
    it("Reverts if called by an approved address", async () => {
      ...
      await expect(nameWrapper)
        ...
        .toBeRevertedWithString("ERC721: approve caller is not token owner or approved for all");
    });
    ...
    it("Allows approved address to call extendExpiry()", async () => {
      const { nameWrapper, accounts, actions } = await loadFixture(approveFixture);
      ...
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), 100n], { account: accounts[1] });
    ...
    it("Does not allows approved address to call setSubnodeOwner()", async () => {
      ...
      await expectOwnerOf(subname).on(nameWrapper).toBe(accounts[1]);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, accounts[2].address, 0, 1000n], { account: accounts[1] })
    ...
    it("Allows approved address to call setSubnodeRecord()", async () => {
      ...
      await expectOwnerOf(subname).on(nameWrapper).toBe(accounts[1]);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [namehash(name), sublabel, accounts[1].address, zeroAddress, 0n, 0, 10000n], {
          account: accounts[1],
        })
        .toBeRevertedWithCustomError("Unauthorised")
        .withArgs(namehash(name), getAddress(accounts[1].address));
    ...
    it("Does not allow approved address to call setChildFuses()", async () => {
      const { nameWrapper, accounts, actions, parentExpiry } = await loadFixture(approveFixture);
    ...
    it("Does not allow approved accounts to extend expiry when expired", async () => {
      const { nameWrapper, accounts, actions, testClient } = await loadFixture(approveFixture);
      await actions.setSubnodeOwner.onNameWrapper({
        parentName: name,
        label: sublabel,
        owner: accounts[1].address,
    ...
    it("Approved address can be replaced and previous approved is removed", async () => {
      ...
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await nameWrapper.write.approve([accounts[2].address, toNameId(name)]);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 500n], {
        account: accounts[2],
      });
      await expect(nameWrapper)
        .write("extendExpiry", [namehash(name), labelhash(sublabel), parentExpiry], { account: accounts[1] })
        .toBeRevertedWithCustomError("Unauthorised")
    ...
    it("Approved address cannot be removed/replaced when fuse is burnt", async () => {
      const { nameWrapper, accounts } = await loadFixture(approveFixture);
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await nameWrapper.write.setFuses([namehash(name), CANNOT_UNWRAP | CANNOT_APPROVE]);
      await expect(nameWrapper)
    ...
    it("Approved address cannot transfer the name with setRecord()", async () => {
      ...
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await expect(nameWrapper)
        .write("setRecord", [namehash(name), accounts[1].address, zeroAddress, 0n], { account: accounts[1] })
        .toBeRevertedWithCustomError("Unauthorised")
        .withArgs(namehash(name), getAddress(accounts[1].address));
    });
    ...
    it("Approved address cannot unwrap .eth", async () => {
      ...
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await expect(nameWrapper)
        .write("unwrapETH2LD", [labelhash(label), accounts[1].address, accounts[1].address], { account: accounts[1] })
        .toBeRevertedWithCustomError("Unauthorised")
        .withArgs(namehash(name), getAddress(accounts[1].address));
    });
    it("Approved address cannot unwrap non .eth", async () => {
      ...
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await expect(nameWrapper)
        .write("unwrap", [namehash(name), labelhash(sublabel), accounts[1].address], { account: accounts[1] })
        .toBeRevertedWithCustomError("Unauthorised")
        .withArgs(namehash(subname), getAddress(accounts[1].address));
    });
    it("Approval is cleared on transfer", async () => {
      const { nameWrapper, accounts } = await loadFixture(approveFixture);
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await nameWrapper.write.safeTransferFrom([accounts[0].address, accounts[2].address, toNameId(name), 1n, "0x"]);
      await expect(nameWrapper.read.getApproved([toNameId(name)])).resolves.toEqualAddress(zeroAddress);
    });
    it("Approval is cleared on unwrapETH2LD()", async () => {
      const { nameWrapper, accounts, actions } = await loadFixture(approveFixture);
      await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
      await expect(nameWrapper.read.getApproved([toNameId(name)])).resolves.toEqualAddress(accounts[1].address);
      await nameWrapper.write.unwrapETH2LD([labelhash(label), accounts[0].address, accounts[0].address]);
      await expect(nameWrapper.read.getApproved([toNameId(name)])).resolves.toEqualAddress(zeroAddress);
    ...
    it("Approval is cleared on unwrap()", async () => {
      ...
      await expect(nameWrapper.read.getApproved([toNameId(subname)])).resolves.toEqualAddress(accounts[1].address);
      await actions.unwrapName({
        parentName: name,
        label: sublabel,
        controller: accounts[0].address,
      });
      await expect(nameWrapper.read.getApproved([toNameId(subname)])).resolves.toEqualAddress(zeroAddress);
