...
import { CANNOT_APPROVE, CANNOT_UNWRAP, CAN_DO_EVERYTHING, CAN_EXTEND_EXPIRY, GRACE_PERIOD, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, } from '../fixtures/utils.js';
export const approveTests = () => {
    describe('approve()', () => {
        ...
        it('Approved address can be replaced and previous approved is removed', async () => {
            ...
            await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
            await nameWrapper.write.approve([accounts[2].address, toNameId(name)]);
            await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 500n], { account: accounts[2] });
            await expect(nameWrapper)
        ...
        it('Approved address cannot transfer the name', async () => {
            ...
            await nameWrapper.write.approve([accounts[1].address, toNameId(name)]);
            await expect(nameWrapper)
                .write('safeTransferFrom', [accounts[0].address, accounts[1].address, toNameId(name), 1n, '0x'], { account: accounts[1] })
