...
export const wrapTests = () =>
  describe("wrap()", () => {
    ...
    it("Allows specifying resolver", async () => {
      ...
      await expect(ensRegistry.read.resolver([namehash(label)])).resolves.toEqualAddress(accounts[1].address);
    });
    it("emits event for NameWrapped", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(namehash("xyz"), dnsEncodeName("xyz"), accounts[0].address, 0, 0n);
    });
    it("emits event for TransferSingle", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(accounts[0].address, zeroAddress, accounts[0].address, toNameId("xyz"), 1n);
    });
    ...
    it("Does not allow wrapping .eth 2LDs.", async () => {
      const { ensRegistry, nameWrapper, baseRegistrar, accounts, actions } = await loadFixture(fixture);
      const label = "wrapped";
      ...
      await actions.setRegistryApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrap", [dnsEncodeName(`${label}.eth`), accounts[1].address, zeroAddress])
    ...
    it("Can re-wrap a name that was reassigned by an unwrapped parent", async () => {
      ...
      await expect(nameWrapper).transaction(tx).toEmitEvent("NameUnwrapped").withArgs(namehash(childName), zeroAddress);
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("TransferSingle")
        .withArgs(accounts[1].address, accounts[0].address, zeroAddress, toNameId(childName), 1n);
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("NameWrapped")
        .withArgs(namehash(childName), dnsEncodeName(childName), accounts[1].address, CAN_DO_EVERYTHING, 0n);
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("TransferSingle")
        .withArgs(accounts[1].address, zeroAddress, accounts[1].address, toNameId(childName), 1n);
    ...
    it("Will not wrap a name with junk at the end", async () => {
      ...
      await actions.setRegistryApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrap", [`${dnsEncodeName("xyz")}123456`, accounts[0].address, zeroAddress])
    ...
    it("Does not allow wrapping a name you do not own", async () => {
      ...
      const nameGriefer = await hre.viem.deployContract("NameGriefer", [nameWrapper.address]);
