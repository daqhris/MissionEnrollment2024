...
export const renewTests = () => {
  describe("renew", () => {
    ...
    it("Renews names", async () => {
      const { baseRegistrar, nameWrapper, accounts } = await loadFixture(renewFixture);
      ...
      const newExpires = await baseRegistrar.read.nameExpires([toLabelId(label)]);
    ...
    it("Renewing name less than required to unexpire it still has original owner/fuses", async () => {
      const { nameWrapper, accounts, testClient, publicClient } = await loadFixture(renewFixture);
      await nameWrapper.write.registerAndWrapETH2LD([
        label,
        accounts[0].address,
        DAY,
      ...
      const [, , expiryBefore] = await nameWrapper.read.getData([toNameId(name)]);
      ...
      const [ownerAfter, fusesAfter, expiryAfter] = await nameWrapper.read.getData([toNameId(name)]);
      expect(ownerAfter).toEqualAddress(accounts[0].address);
      // fuses remain the same
      expect(fusesAfter).toEqual(CANNOT_UNWRAP | CANNOT_SET_RESOLVER | IS_DOT_ETH | PARENT_CANNOT_CONTROL);
      // still expired
