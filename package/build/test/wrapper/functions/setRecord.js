...
import { CANNOT_SET_RESOLVER, CANNOT_SET_TTL, CANNOT_TRANSFER, CANNOT_UNWRAP, CAN_DO_EVERYTHING, MAX_EXPIRY, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, zeroAccount, } from '../fixtures/utils.js';
export const setRecordTests = () => {
    describe('setRecord', () => {
        ...
        it('Performs the appropriate function on the ENS registry and Wrapper', async () => {
            const { ensRegistry, nameWrapper, accounts } = await loadFixture(setRecordFixture);
            await nameWrapper.write.setRecord([
                namehash(name),
                accounts[1].address,
                accounts[0].address,
            ...
            await expect(ensRegistry.read.resolver([namehash(name)])).resolves.toEqualAddress(accounts[0].address);
        ...
        it('Can be called by an account authorised by the owner.', async () => {
            ...
            await nameWrapper.write.setRecord([namehash(name), accounts[1].address, accounts[0].address, 50n], { account: accounts[1] });
        });
        it('Cannot be called by anyone else.', async () => {
            const { nameWrapper, accounts } = await loadFixture(setRecordFixture);
            await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
            await expect(nameWrapper)
                .write('setRecord', [namehash(name), accounts[1].address, accounts[0].address, 50n], { account: accounts[1] })
        ...
        it('Setting the owner of a subdomain to 0 unwraps the name and passes through resolver/ttl', async () => {
            ...
            await expect(ensRegistry.read.ttl([namehash(subname)])).resolves.toEqual(50n);
        });
