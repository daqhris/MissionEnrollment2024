...
export const registerAndWrapETH2LDTests = () => {
  describe("registerAndWrapETH2LD()", () => {
    ...
    it("should register and wrap names", async () => {
      const { ensRegistry, baseRegistrar, nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
    ...
    it("allows specifying a resolver address", async () => {
      const { ensRegistry, nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
      await nameWrapper.write.registerAndWrapETH2LD([
        label,
        accounts[0].address,
        86400n,
    ...
    it("does not allow non controllers to register names", async () => {
      const { nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
      await nameWrapper.write.setController([accounts[0].address, false]);
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", [label, accounts[0].address, 86400n, zeroAddress, CAN_DO_EVERYTHING])
    ...
    it("Does not allow wrapping with a target address of 0x0", async () => {
      ...
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", [label, zeroAddress, 86400n, zeroAddress, CAN_DO_EVERYTHING])
    ...
    it("Does not allow wrapping with a target address of the wrapper contract address.", async () => {
      const { nameWrapper } = await loadFixture(registerAndWrapETH2LDFixture);
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", [label, nameWrapper.address, 86400n, zeroAddress, CAN_DO_EVERYTHING])
    ...
    it("Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.", async () => {
      const { nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", [label, accounts[0].address, 86400n, zeroAddress, CANNOT_SET_RESOLVER])
    ...
    it("Allows fuse to be burned if CANNOT_UNWRAP has been burned and expiry set", async () => {
      ...
      await nameWrapper.write.registerAndWrapETH2LD([label, accounts[0].address, 86400n, zeroAddress, initialFuses]);
    ...
    it("Errors when adding a number greater than uint16 for fuses", async () => {
      ...
      await expect(nameWrapper).transaction(walletClient.sendTransaction(tx)).toBeRevertedWithoutReason();
    });
    it("Errors when passing a parent-controlled fuse", async () => {
      const { nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
      for (let i = 0; i < 7; i++) {
        await expect(nameWrapper)
          .write("registerAndWrapETH2LD", [label, accounts[0].address, 86400n, zeroAddress, IS_DOT_ETH * 2 ** i])
    ...
    it("Will not wrap a name with an empty label", async () => {
      const { nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", ["", accounts[0].address, 86400n, zeroAddress, CAN_DO_EVERYTHING])
    ...
    it("Will not wrap a name with a label more than 255 characters", async () => {
      ...
      await expect(nameWrapper)
        .write("registerAndWrapETH2LD", [longString, accounts[0].address, 86400n, zeroAddress, CAN_DO_EVERYTHING])
        .toBeRevertedWithCustomError("LabelTooLong")
        .withArgs(longString);
    });
    it("emits Wrap event", async () => {
      const { baseRegistrar, nameWrapper, accounts } = await loadFixture(registerAndWrapETH2LDFixture);
