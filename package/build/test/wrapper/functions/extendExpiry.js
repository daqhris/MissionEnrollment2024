...
export const extendExpiryTests = () => {
  describe("extendExpiry()", () => {
    ...
    it("Allows parent owner to set expiry without CAN_EXTEND_EXPIRY burned", async () => {
      ...
      const [, initialFuses, initialExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), MAX_EXPIRY]);
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Allows parent owner to set expiry with CAN_EXTEND_EXPIRY burned", async () => {
      ...
      const [, initialFuses, initialExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), MAX_EXPIRY]);
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(newExpiry).toEqual(parentExpiry + GRACE_PERIOD);
    });
    ...
    it("Allows approved operators of parent owner to set expiry without CAN_EXTEND_EXPIRY burned", async () => {
      ...
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), MAX_EXPIRY], { account: accounts[2] });
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP);
      expect(newExpiry).toEqual(parentExpiry + GRACE_PERIOD);
    });
    ...
    it("Does not allow child owner to set expiry without CAN_EXTEND_EXPIRY burned", async () => {
      ...
      await expect(nameWrapper)
        .write("extendExpiry", [namehash(name), labelhash(sublabel), MAX_EXPIRY], { account: accounts[1] })
    ...
    it("Allows child owner to set expiry with CAN_EXTEND_EXPIRY burned", async () => {
      ...
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), MAX_EXPIRY], { account: accounts[1] });
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(newExpiry).toEqual(parentExpiry + GRACE_PERIOD);
    });
    it("Does not allow approved operator of child owner to set expiry without CAN_EXTEND_EXPIRY burned", async () => {
      ...
      await expect(nameWrapper)
        .write("extendExpiry", [namehash(name), labelhash(sublabel), MAX_EXPIRY], { account: accounts[2] })
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
    ...
    it("Does not allow owner of .eth 2LD to set expiry", async () => {
      ...
      const [, initialFuses, expiry] = await nameWrapper.read.getData([toNameId(name)]);
      expect(initialFuses).toEqual(IS_DOT_ETH | PARENT_CANNOT_CONTROL | CANNOT_UNWRAP);
      await expect(nameWrapper)
    ...
    it("Expiry is normalized to old expiry if too low", async () => {
      ...
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 3601n], {
        account: accounts[1],
      });
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
    ...
    it("Expiry is normalized to parent expiry if too high", async () => {
      ...
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry + GRACE_PERIOD + 1n], {
        account: accounts[1],
      });
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
    ...
    it("Expiry is not normalized to new value if between old expiry and parent expiry", async () => {
      ...
      expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
      expect(initialExpiry).toEqual(parentExpiry - 3600n);
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 1800n], {
        account: accounts[1],
      });
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
    ...
    it("Does not allow .eth 2LD owner to set expiry on child if the .eth 2LD is expired but grace period has not ended", async () => {
      const { baseRegistrar, nameWrapper, testClient, actions, accounts } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      await expect(nameWrapper)
        .write("extendExpiry", [namehash(name), labelhash(sublabel), MAX_EXPIRY])
        .toBeRevertedWithCustomError("Unauthorised")
    ...
    it("Does not allow parent owner to set expiry if Emancipated child name has expired", async () => {
      ...
      const [owner, initialFuses, initialExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Allows parent owner to set expiry if non-Emancipated child name has reached its expiry", async () => {
      ...
      await testClient.mine({ blocks: 1 });
      await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), MAX_EXPIRY]);
      const [owner, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
      expect(owner).toEqualAddress(accounts[1].address);
      expect(newFuses).toEqual(0);
      expect(newExpiry).toEqual(parentExpiry + GRACE_PERIOD);
    });
    ...
    it("Does not allow extendExpiry() to be called on unregistered names (expired w/ PCC burnt)", async () => {
      const { baseRegistrar, nameWrapper, accounts, actions, testClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
        duration: 10n * DAY,
    ...
    it("Does not allow extendExpiry() to be called on unwrapped names", async () => {
      const { ensRegistry, baseRegistrar, nameWrapper, actions, accounts } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: 0,
      });
