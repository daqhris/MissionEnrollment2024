...
import { CANNOT_UNWRAP, CAN_EXTEND_EXPIRY, GRACE_PERIOD, IS_DOT_ETH, MAX_EXPIRY, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, } from '../fixtures/utils.js';
export const extendExpiryTests = () => {
    describe('extendExpiry()', () => {
        ...
        it('Expiry is normalized to old expiry if too low', async () => {
            ...
            expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
            expect(initialExpiry).toEqual(parentExpiry - 3600n);
            await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 3601n], { account: accounts[1] });
            const [, newFuses, newExpiry] = await nameWrapper.read.getData([
                toNameId(subname),
            ]);
            expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
        ...
        it('Expiry is normalized to parent expiry if too high', async () => {
            ...
            expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
            expect(initialExpiry).toEqual(parentExpiry - 3600n);
            await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry + GRACE_PERIOD + 1n], { account: accounts[1] });
            const [, newFuses, newExpiry] = await nameWrapper.read.getData([
                toNameId(subname),
            ]);
            expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
        ...
        it('Expiry is not normalized to new value if between old expiry and parent expiry', async () => {
            ...
            expect(initialFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
            expect(initialExpiry).toEqual(parentExpiry - 3600n);
            await nameWrapper.write.extendExpiry([namehash(name), labelhash(sublabel), parentExpiry - 1800n], { account: accounts[1] });
            const [, newFuses, newExpiry] = await nameWrapper.read.getData([
                toNameId(subname),
            ]);
            expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CAN_EXTEND_EXPIRY);
