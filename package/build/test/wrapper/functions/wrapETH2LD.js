...
import { getAddress, keccak256, namehash, stringToBytes, zeroAddress } from "viem";
...
export const wrapETH2LDTests = () =>
  describe("wrapETH2LD()", () => {
    ...
    it("wraps a name if sender is owner", async () => {
      const { ensRegistry, baseRegistrar, nameWrapper, accounts, actions } = await loadFixture(fixture);
    ...
    it("Cannot wrap a name if the owner has not authorised the wrapper with the .eth registrar.", async () => {
      ...
      await expect(nameWrapper)
        .write("wrapETH2LD", [label, accounts[0].address, CAN_DO_EVERYTHING, zeroAddress])
    ...
    it("Allows specifying resolver", async () => {
      ...
      await expect(ensRegistry.read.resolver([namehash(name)])).resolves.toEqualAddress(accounts[1].address);
    });
    it("Can re-wrap a name that was wrapped has already expired on the .eth registrar", async () => {
      const { baseRegistrar, nameWrapper, accounts, testClient, actions } = await loadFixture(fixture);
      await actions.register({
        label,
        owner: accounts[0].address,
        duration: 1n * DAY,
      ...
      await expect(baseRegistrar.read.available([toLabelId(label)])).resolves.toBe(true);
      await actions.register({
        label,
      ...
      await expect(nameWrapper).transaction(tx).toEmitEvent("NameUnwrapped").withArgs(namehash(name), zeroAddress);
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("TransferSingle")
        .withArgs(accounts[1].address, accounts[0].address, zeroAddress, toNameId(name), 1n);
      await expect(nameWrapper)
        .transaction(tx)
      ...
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("TransferSingle")
        .withArgs(accounts[1].address, zeroAddress, accounts[1].address, toNameId(name), 1n);
    ...
    it("correctly reports fuses for a name that has expired and been rewrapped more permissively", async () => {
      ...
      expect(initialFuses).toEqual(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | IS_DOT_ETH);
      ...
      const [, subFuses] = await nameWrapper.read.getData([toNameId("sub." + name)]);
      ...
      const expectedExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]).then(e => e + GRACE_PERIOD);
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(name)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | IS_DOT_ETH);
      expect(newExpiry).toEqual(expectedExpiry);
      // subdomain fuses get reset
      const [, newSubFuses] = await nameWrapper.read.getData([toNameId("sub." + name)]);
    ...
    it("correctly reports fuses for a name that has expired and been rewrapped more permissively with registerAndWrap()", async () => {
      ...
      await nameWrapper.write.registerAndWrapETH2LD([label, accounts[1].address, 1n * DAY, zeroAddress, 0]);
      const expectedExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]).then(e => e + GRACE_PERIOD);
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(name)]);
      expect(newFuses).toEqual(PARENT_CANNOT_CONTROL | IS_DOT_ETH);
      expect(newExpiry).toEqual(expectedExpiry);
    ...
    it("emits Wrap event", async () => {
      const { baseRegistrar, nameWrapper, accounts, actions } = await loadFixture(fixture);
      await actions.register({
        label,
        owner: accounts[0].address,
        duration: 1n * DAY,
    ...
    it("emits TransferSingle event", async () => {
      ...
      await expect(nameWrapper)
        .transaction(tx)
        .toEmitEvent("TransferSingle")
        .withArgs(accounts[0].address, zeroAddress, accounts[0].address, toNameId(name), 1n);
    });
    ...
    it("Does not allow wrapping with a target address of 0x0", async () => {
      ...
      await actions.setBaseRegistrarApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrapETH2LD", [label, zeroAddress, CAN_DO_EVERYTHING, zeroAddress])
    ...
    it("Does not allow wrapping with a target address of the wrapper contract address.", async () => {
      ...
      await actions.setBaseRegistrarApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrapETH2LD", [label, nameWrapper.address, CAN_DO_EVERYTHING, zeroAddress])
    ...
    it("Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.", async () => {
      ...
      await actions.setBaseRegistrarApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrapETH2LD", [label, accounts[0].address, CANNOT_SET_RESOLVER, zeroAddress])
    ...
    it("Allows fuse to be burned if CANNOT_UNWRAP has been burned, but resets to 0 if expired", async () => {
      const { nameWrapper, accounts, testClient, actions } = await loadFixture(fixture);
      await actions.register({
        label,
        owner: accounts[0].address,
        duration: 1n * DAY,
    ...
    it("Will not wrap an empty name", async () => {
      ...
      await baseRegistrar.write.register([toTokenId(emptyLabelhash), accounts[0].address, 1n * DAY]);
      await actions.setBaseRegistrarApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrapETH2LD", ["", accounts[0].address, CAN_DO_EVERYTHING, zeroAddress])
    ...
    it("Will not wrap a label greater than 255 characters", async () => {
      ...
      await actions.setBaseRegistrarApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrapETH2LD", [longString, accounts[0].address, CAN_DO_EVERYTHING, zeroAddress])
