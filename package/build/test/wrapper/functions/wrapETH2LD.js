...
export const wrapETH2LDTests = () => describe('wrapETH2LD()', () => {
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .eth registrar', async () => {
        ...
        await expect(nameWrapper)
            ...
            .withArgs(namehash(name), dnsEncodeName(name), accounts[1].address, PARENT_CANNOT_CONTROL | IS_DOT_ETH, expectedExpiry + GRACE_PERIOD);
        await expect(nameWrapper)
            .transaction(tx)
            .toEmitEvent('TransferSingle')
    ...
    it('emits Wrap event', async () => {
        ...
        await expect(nameWrapper)
            .transaction(tx)
            .toEmitEvent('NameWrapped')
            .withArgs(namehash(name), dnsEncodeName(name), accounts[0].address, CAN_DO_EVERYTHING | IS_DOT_ETH, expiry + GRACE_PERIOD);
    });
