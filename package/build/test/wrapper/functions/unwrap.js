  ...
  describe("unwrap()", () => {
    ...
    it("Will not allow previous owner to unwrap name when name expires", async () => {
      ...
      await expect(nameWrapper)
        .write("unwrap", [namehash(parentName), labelhash(childLabel), accounts[0].address])
    ...
    it("emits TransferSingle event", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(accounts[0].address, accounts[0].address, zeroAddress, toNameId(label), 1n);
    });
    ...
    it("Does not allow an account authorised by the owner on the ENS registry to unwrap a name", async () => {
      ...
      await expectOwnerOf(label).on(ensRegistry).toBe(accounts[1]);
      await expect(ensRegistry.read.isApprovedForAll([accounts[1].address, accounts[0].address])).resolves.toBe(true);
      // wrap using accounts[0]
      await actions.wrapName({
        name: label,
        owner: accounts[1].address,
    ...
    it("Will not unwrap .eth 2LDs.", async () => {
      const { nameWrapper, baseRegistrar, accounts, actions } = await loadFixture(fixture);
      ...
      await expect(nameWrapper)
        .write("unwrap", [namehash("eth"), labelhash(label), accounts[0].address])
    ...
    it("Will not allow to unwrap with PCC/CU burned if expired", async () => {
      const { accounts, ensRegistry, nameWrapper, actions } = await loadFixture(fixture);
    ...
    it("Will allow to unwrap with PCC/CU burned if expired and then extended without PCC/CU", async () => {
      const { baseRegistrar, nameWrapper, ensRegistry, accounts, publicClient, testClient, actions } =
    ...
    it("Unwrapping a previously wrapped unexpired name retains PCC and expiry", async () => {
      ...
      const parentExpiry = await baseRegistrar.read.nameExpires([toLabelId(parentLabel)]);
      ...
      const [, fusesBefore] = await nameWrapper.read.getData([toNameId(childName)]);
      ...
      const [, fusesAfter, expiryAfter] = await nameWrapper.read.getData([toNameId(childName)]);
