...
import { CANNOT_UNWRAP, CAN_DO_EVERYTHING, GRACE_PERIOD, IS_DOT_ETH, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, } from '../fixtures/utils.js';
export const unwrapETH2LDTests = () => describe('unwrapETH2LD()', () => {
    const label = 'unwrapped';
    const name = `${label}.eth`;
    it('Allows the owner to unwrap a name.', async () => {
        const { baseRegistrar, ensRegistry, nameWrapper, accounts, actions } = await loadFixture(fixture);
    ...
    it('Emits TransferSingle event', async () => {
        ...
        await expect(nameWrapper)
            ...
            .withArgs(accounts[0].address, accounts[0].address, zeroAddress, toNameId(name), 1n);
    });
    it('Does not allows an account authorised by the owner on the .eth registrar to unwrap a name', async () => {
        ...
        await expect(nameWrapper)
            .write('unwrapETH2LD', [labelhash(label), accounts[1].address, accounts[1].address], {
            account: accounts[1],
        })
            .toBeRevertedWithCustomError('Unauthorised')
