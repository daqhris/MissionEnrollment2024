...
export const setChildFusesTests = () => {
  describe("setChildFuses()", () => {
    ...
    it("Allows special cased TLD owners to set fuses/expiry", async () => {
      const { nameWrapper, actions, accounts, publicClient } = await loadFixture(fixture);
      ...
      const [, fuses, expiry] = await nameWrapper.read.getData([toNameId("anothertld")]);
    ...
    it("does not allow parent owners to burn IS_DOT_ETH fuse", async () => {
      ...
      const [, fuses, expiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Does not allow parent owners to burn parent controlled fuses after burning PCC", async () => {
      ...
      expect(fuses).toEqual(0);
      expect(expiry).toEqual(0n);
      await nameWrapper.write.setChildFuses([namehash(name), labelhash(sublabel), PARENT_CANNOT_CONTROL, MAX_EXPIRY]);
      await expect(nameWrapper)
        .write("setChildFuses", [
          namehash(name),
          labelhash(sublabel),
    ...
    it("Allows accounts authorised by the parent node owner to set fuses/expiry", async () => {
      ...
      await nameWrapper.write.setChildFuses(
        [namehash(name), labelhash(sublabel), CANNOT_UNWRAP | PARENT_CANNOT_CONTROL, MAX_EXPIRY],
        { account: accounts[1] },
      );
      const expectedExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]);
      const [, newFuses, newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Normalises expiry to the parent expiry", async () => {
      ...
      const [, , expectedExpiry] = await nameWrapper.read.getData([toNameId(name)]);
      const [, , newExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Does not allow burning fuses if PARENT_CANNOT_CONTROL is not burnt", async () => {
      ...
      await expect(nameWrapper)
        .write("setChildFuses", [namehash(name), labelhash(sublabel), CANNOT_UNWRAP, MAX_EXPIRY])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
    it("should not allow .eth to call setChildFuses()", async () => {
      ...
      await expect(nameWrapper)
        .write("setChildFuses", [namehash("eth"), labelhash(label), CANNOT_SET_RESOLVER, 0n])
        .toBeRevertedWithCustomError("Unauthorised")
    ...
    it("Does not allow burning fuses if CANNOT_UNWRAP is not burnt", async () => {
      ...
      await expect(nameWrapper)
        .write("setChildFuses", [namehash(name), labelhash(sublabel), CANNOT_SET_RESOLVER, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
    it("Does not allow burning fuses if PARENT_CANNOT_CONTROL is already burned", async () => {
      ...
      await nameWrapper.write.setChildFuses([namehash(name), labelhash(sublabel), originalFuses, MAX_EXPIRY]);
      await expect(nameWrapper)
        .write("setChildFuses", [
          namehash(name),
          labelhash(sublabel),
    ...
    it("Does not allow burning fuses if PARENT_CANNOT_CONTROL is already burned even if PARENT_CANNOT_CONTROL is added as a fuse", async () => {
      ...
      await expect(nameWrapper)
        .write("setChildFuses", [
          namehash(name),
          labelhash(sublabel),
          PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CANNOT_SET_RESOLVER | CANNOT_BURN_FUSES,
          MAX_EXPIRY,
        ])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    ...
    it("Does not allow burning PARENT_CANNOT_CONTROL if CU on the parent is not burned", async () => {
      ...
      const originalFuses = PARENT_CANNOT_CONTROL | CANNOT_UNWRAP;
      await expect(nameWrapper)
        .write("setChildFuses", [namehash(name), labelhash(sublabel), originalFuses, MAX_EXPIRY])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
