...
export const setSubnodeOwnerTests = () =>
  describe("setSubnodeOwner()", () => {
    ...
    it("Can be called by the owner of a name and sets this contract as owner on the ENS registry.", async () => {
      const { ensRegistry, nameWrapper, actions, accounts } = await loadFixture(setSubnodeOwnerFixture);
    ...
    it("Will not allow wrapping with a target address of 0x0.", async () => {
      const { nameWrapper, accounts } = await loadFixture(setSubnodeOwnerFixture);
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, zeroAddress, CAN_DO_EVERYTHING, 0n])
    ...
    it("Will not allow wrapping with a target address of the wrapper contract address", async () => {
      ...
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, nameWrapper.address, CAN_DO_EVERYTHING, 0n])
        .toBeRevertedWithString("ERC1155: newOwner cannot be the NameWrapper contract");
    });
    it("Does not allow anyone else to wrap a name even if the owner has authorised the wrapper with the ENS registry.", async () => {
      ...
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, accounts[0].address, CAN_DO_EVERYTHING, 0n], {
          account: accounts[1],
        })
    ...
    it("Does not allow IS_DOT_ETH to be burned", async () => {
      ...
      await expect(nameWrapper)
        .write("setSubnodeOwner", [
          ...
          sublabel,
          accounts[0].address,
          CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER | IS_DOT_ETH,
          MAX_EXPIRY,
        ])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    ...
    it("Does not allow fuses to be burned if CANNOT_UNWRAP and PARENT_CANNOT_CONTROL are burned, but the name is expired", async () => {
      ...
      expect(parentFuses).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | IS_DOT_ETH);
      await actions.setSubnodeOwner.onNameWrapper({
        parentName: name,
        label: sublabel,
        owner: accounts[0].address,
      ...
      await expect(nameWrapper.read.allFusesBurned([namehash(subname), PARENT_CANNOT_CONTROL])).resolves.toBe(false);
    });
    it("normalises the max expiry of a subdomain to the parent's expiry", async () => {
      // note: not using suite specific fixture here
      const { baseRegistrar, nameWrapper, actions, accounts } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CAN_DO_EVERYTHING | CANNOT_UNWRAP,
      });
      const expectedExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]);
      await actions.setSubnodeOwner.onNameWrapper({
        parentName: name,
        label: sublabel,
        owner: accounts[0].address,
    ...
    it("Emits Wrap event", async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(setSubnodeOwnerFixture);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, accounts[1].address, 0, 0n])
        .toEmitEvent("NameWrapped")
        .withArgs(namehash(subname), dnsEncodeName(subname), accounts[1].address, 0, 0n);
    });
    it("Emits TransferSingle event", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(accounts[0].address, zeroAddress, accounts[1].address, toNameId(subname), 1n);
    });
    it("Will not create a subdomain with an empty label", async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(setSubnodeOwnerFixture);
      await actions.setRegistryApprovalForWrapper();
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), "", accounts[0].address, CAN_DO_EVERYTHING, 0n])
    ...
    it("setting owner to 0 burns and unwraps", async () => {
      ...
      await expect(nameWrapper).transaction(tx).toEmitEvent("NameUnwrapped").withArgs(namehash(subname), zeroAddress);
    });
    it("Unwrapping within an external contract does not create any state inconsistencies", async () => {
      ...
      const testReentrancy = await hre.viem.deployContract("TestNameWrapperReentrancy", [
        accounts[0].address,
        nameWrapper.address,
        namehash("test.eth"),
        labelhash("sub"),
    ...
    it("Unwrapping a previously wrapped unexpired name retains PCC and so reverts setSubnodeRecord", async () => {
      // note: not using suite specific fixture here
      const { nameWrapper, actions, accounts, baseRegistrar } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      const parentExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]);
      // Confirm that the name is wrapped
      await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0]);
      // NameWrapper.setSubnodeOwner to accounts[1]
      await actions.setSubnodeOwner.onNameWrapper({
      ...
      const [, fusesBefore] = await nameWrapper.read.getData([toNameId(subname)]);
      ...
      const [owner, fuses, expiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Rewrapping a name that had PCC burned, but has now expired is possible and resets fuses", async () => {
      // note: not using suite specific fixture here
      const { nameWrapper, actions, accounts, baseRegistrar, testClient, publicClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      const [, fusesAfter, expiryAfter] = await nameWrapper.read.getData([toNameId(subname)]);
      ...
      await expectOwnerOf(subname).on(nameWrapper).toBe(accounts[1]);
      const [rawOwner, rawFuses, expiry2] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent", async () => {
      ...
      await nameWrapper.write.setFuses([namehash(name), CANNOT_CREATE_SUBDOMAIN]);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel2, accounts[1].address, 0, parentExpiry - DAY / 2n])
        .toBeRevertedWithCustomError("OperationProhibited")
      ...
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, accounts[1].address, 0, parentExpiry - DAY / 2n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
    it("Burning a name still protects it from the parent as long as it is unexpired and has PCC burnt", async () => {
      // note: not using suite specific fixture here
      const { ensRegistry, nameWrapper, actions, accounts, baseRegistrar, publicClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      await expect(nameWrapper)
        .write("setSubnodeOwner", [namehash(name), sublabel, accounts[0].address, PARENT_CANNOT_CONTROL, MAX_EXPIRY])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(subname));
    });
  });
