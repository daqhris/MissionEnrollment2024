...
import { CANNOT_SET_RESOLVER, CANNOT_UNWRAP, expectOwnerOf, deployNameWrapperWithUtils as fixture, } from '../fixtures/utils.js';
export const setResolverTests = () => {
    describe('setResolver', () => {
        ...
        it('Performs the appropriate function on the ENS registry.', async () => {
            const { ensRegistry, nameWrapper, accounts } = await loadFixture(setResolverFixture);
            await expect(ensRegistry.read.resolver([namehash(name)])).resolves.toEqualAddress(zeroAddress);
            await nameWrapper.write.setResolver([namehash(name), accounts[1].address]);
