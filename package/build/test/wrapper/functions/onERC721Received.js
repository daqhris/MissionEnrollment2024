...
import { CANNOT_TRANSFER, CANNOT_UNWRAP, GRACE_PERIOD, IS_DOT_ETH, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, zeroAccount, } from '../fixtures/utils.js';
export const onERC721ReceivedTests = () => {
    describe('onERC721Received', () => {
        const label = 'send2contract';
        const name = `${label}.eth`;
        const encodeExtraData = ({ label, owner, ownerControlledFuses, resolver, }) => encodeAbiParameters([
            { type: 'string' },
            { type: 'address' },
            { type: 'uint16' },
            { type: 'address' },
        ...
        it('Allows burning other fuses if CAN_UNWRAP has been burnt', async () => {
            ...
            await expect(nameWrapper.read.allFusesBurned([
                namehash(name),
        ...
        it('Allows burning other fuses if CAN_UNWRAP has been burnt, but resets fuses if expired', async () => {
            const { baseRegistrar, ensRegistry, nameWrapper, accounts, testClient } = await loadFixture(onERC721ReceivedFixture);
            await ensRegistry.write.setOwner([namehash(name), accounts[1].address]);
            await baseRegistrar.write.safeTransferFrom([
                accounts[0].address,
                nameWrapper.address,
        ...
        it('emits NameWrapped Event', async () => {
            ...
            await expect(nameWrapper)
                ...
                .withArgs(namehash(name), dnsEncodeName(name), accounts[0].address, CANNOT_UNWRAP | CANNOT_TRANSFER | PARENT_CANNOT_CONTROL | IS_DOT_ETH, expectedExpiry + GRACE_PERIOD);
        });
