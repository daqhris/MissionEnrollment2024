...
import { encodeAbiParameters, encodeFunctionData, keccak256, labelhash, namehash, zeroAddress } from "viem";

export const onERC721ReceivedTests = () => {
  describe("onERC721Received", () => {
    const label = "send2contract";
    const name = `${label}.eth`;
    const encodeExtraData = ({ label, owner, ownerControlledFuses, resolver }) =>
      encodeAbiParameters(
        [{ type: "string" }, { type: "address" }, { type: "uint16" }, { type: "address" }],
    ...
    it("Wraps a name transferred to it and sets the owner to the provided address", async () => {
      const { baseRegistrar, nameWrapper, accounts } = await loadFixture(onERC721ReceivedFixture);
    ...
    it("Reverts if called by anything other than the ENS registrar address", async () => {
      const { nameWrapper, accounts } = await loadFixture(onERC721ReceivedFixture);
    ...
    it("Accepts fuse values from the data field", async () => {
      ...
      await expect(nameWrapper.read.allFusesBurned([namehash(name), CANNOT_UNWRAP])).resolves.toEqual(true);
    });
    it("Allows specifiying resolver address", async () => {
      const { baseRegistrar, nameWrapper, ensRegistry, accounts } = await loadFixture(onERC721ReceivedFixture);
      await baseRegistrar.write.safeTransferFrom([
        accounts[0].address,
        nameWrapper.address,
        toLabelId(label),
    ...
    it("Reverts if transferred without data", async () => {
      ...
      await expect(baseRegistrar)
        .write("safeTransferFrom", [accounts[0].address, nameWrapper.address, toLabelId(label), "0x"])
        .toBeRevertedWithString("ERC721: transfer to non ERC721Receiver implementer");
    });
    ...
    it("Reverts if CANNOT_UNWRAP is not burned and attempts to burn other fuses", async () => {
      const { baseRegistrar, ensRegistry, nameWrapper, accounts } = await loadFixture(onERC721ReceivedFixture);
    ...
    it("Reverts when manually changing fuse calldata to incorrect type", async () => {
      ...
      data = data.replace("00111", rogueFuse);
      const tx = {
    ...
    it("Allows burning other fuses if CAN_UNWRAP has been burnt", async () => {
      ...
      await expect(
        nameWrapper.read.allFusesBurned([namehash(name), CANNOT_UNWRAP | CANNOT_TRANSFER | PARENT_CANNOT_CONTROL]),
      ).resolves.toEqual(true);
    });
    it("Allows burning other fuses if CAN_UNWRAP has been burnt, but resets fuses if expired", async () => {
      const { baseRegistrar, ensRegistry, nameWrapper, accounts, testClient } = await loadFixture(
      ...
      await expect(
        nameWrapper.read.allFusesBurned([namehash(name), CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_TRANSFER]),
    ...
    it("emits TransferSingle Event", async () => {
      ...
      await expect(nameWrapper)
        ...
        .withArgs(baseRegistrar.address, zeroAddress, accounts[0].address, toNameId(name), 1n);
    });
    it("will not wrap a name with an empty label", async () => {
      const { baseRegistrar, nameWrapper, accounts } = await loadFixture(fixture);
      const emptyLabelId = toTokenId(keccak256(new Uint8Array(0)));
      await baseRegistrar.write.register([emptyLabelId, accounts[0].address, 1n * DAY]);
      const tx = baseRegistrar.write.safeTransferFrom([
        accounts[0].address,
        nameWrapper.address,
        emptyLabelId,
      ...
      await expect(nameWrapper).transaction(tx).toBeRevertedWithCustomError("LabelTooShort");
    });
  });
};
