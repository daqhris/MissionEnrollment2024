...
import { CANNOT_SET_RESOLVER, CANNOT_TRANSFER, CANNOT_UNWRAP, CAN_DO_EVERYTHING, GRACE_PERIOD, IS_DOT_ETH, MAX_EXPIRY, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, zeroAccount, } from '../fixtures/utils.js';
export const upgradeTests = () => describe('upgrade()', () => {
    describe('.eth', () => {
        const label = 'wrapped2';
        const name = `${label}.eth`;
        it('Upgrades a .eth name if sender is owner', async () => {
            const { nameWrapper, baseRegistrar, ensRegistry, nameWrapperUpgraded, actions, accounts, } = await loadFixture(fixture);
            ...
            await expect(nameWrapper)
                ...
                .withArgs(dnsEncodeName(name), accounts[0].address, PARENT_CANNOT_CONTROL | IS_DOT_ETH, expectedExpiry + GRACE_PERIOD, zeroAddress, '0x00');
        });
        ...
        it('Cannot upgrade a name if the upgradeContract has been set and then set back to the 0 address.', async () => {
            const { nameWrapper, nameWrapperUpgraded, actions } = await loadFixture(fixture);
            await actions.registerSetupAndWrapName({
                label,
                fuses: CAN_DO_EVERYTHING,
            });
            ...
            await expect(nameWrapper.read.upgradeContract()).resolves.toEqualAddress(nameWrapperUpgraded.address);
        ...
        it('Will pass fuses and expiry to the upgradedContract without any changes.', async () => {
            const { nameWrapper, baseRegistrar, nameWrapperUpgraded, actions, accounts, } = await loadFixture(fixture);
            await actions.registerSetupAndWrapName({
                label,
            ...
            await expect(nameWrapper)
                .write('upgrade', [dnsEncodeName(name), '0x'])
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(name), accounts[0].address, PARENT_CANNOT_CONTROL |
                CANNOT_UNWRAP |
                CANNOT_SET_RESOLVER |
                IS_DOT_ETH, expectedExpiry, zeroAddress, '0x00');
        });
        ...
        it('will revert if called twice by the original owner', async () => {
            const { nameWrapper, nameWrapperUpgraded, actions, accounts } = await loadFixture(fixture);
            await actions.registerSetupAndWrapName({
                label,
                fuses: CANNOT_UNWRAP,
            });
        ...
        it('Will allow you to pass through extra data on upgrade', async () => {
            ...
            await expect(nameWrapper)
                ...
                CANNOT_UNWRAP |
                CANNOT_SET_RESOLVER |
                IS_DOT_ETH, expectedExpiry, zeroAddress, '0x01');
        });
    ...
    describe('other', () => {
        ...
        it('Allows owner to upgrade name', async () => {
            ...
            await expect(nameWrapper)
                .write('upgrade', [dnsEncodeName(name), '0x'])
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(name), accounts[0].address, 0, 0n, zeroAddress, '0x00');
        });
        it('upgrades a name if sender is authorized by the owner', async () => {
            ...
            await expect(nameWrapper)
                ...
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(xyzName), accounts[0].address, 0, 0n, zeroAddress, '0x00');
        });
        it('Cannot upgrade a name if the upgradeContract has not been set.', async () => {
        ...
        it('Will pass fuses and expiry to the upgradedContract without any changes.', async () => {
            const { nameWrapper, nameWrapperUpgraded, actions, accounts, baseRegistrar, } = await loadFixture(fixture);
            await actions.registerSetupAndWrapName({
                label: parentLabel,
                fuses: CANNOT_UNWRAP,
            });
            await actions.setRegistryApprovalForWrapper();
            const expectedFuses = PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CANNOT_TRANSFER;
            await actions.setSubnodeOwner.onNameWrapper({
                parentName,
                label,
                owner: accounts[0].address,
            ...
            await expect(nameWrapper)
                .write('upgrade', [dnsEncodeName(name), '0x'])
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(name), accounts[0].address, expectedFuses, expectedExpiry, zeroAddress, '0x00');
        });
        ...
        it('Keeps approval information on upgrade', async () => {
            ...
            await expect(nameWrapper.read.getApproved([toNameId(xyzName)])).resolves.toEqualAddress(accounts[2].address);
            // set the upgradeContract of the NameWrapper contract
            await nameWrapper.write.setUpgradeContract([
                nameWrapperUpgraded.address,
            ]);
            await expect(nameWrapper)
                .write('upgrade', [dnsEncodeName(xyzName), '0x'])
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(xyzName), accounts[0].address, 0, 0n, accounts[2].address, '0x');
        });
        it('Will allow you to pass through extra data on upgrade', async () => {
            const { nameWrapper, nameWrapperUpgraded, actions, accounts } = await loadFixture(fixture);
            const xyzName = `${label}.xyz`;
            ...
            await expect(nameWrapper)
                ...
                .toEmitEventFrom(nameWrapperUpgraded, 'NameUpgraded')
                .withArgs(dnsEncodeName(xyzName), accounts[0].address, 0, 0n, zeroAddress, '0x01');
        });
        it('Does not allow anyone else to upgrade a name even if the owner has authorised the wrapper with the ENS registry.', async () => {
            const { nameWrapper, nameWrapperUpgraded, actions, accounts } = await loadFixture(fixture);
            const xyzName = `${label}.xyz`;
