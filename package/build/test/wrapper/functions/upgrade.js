  ...
  describe("upgrade()", () => {
    describe(".eth", () => {
      ...
      it("Upgrades a .eth name if sender is owner", async () => {
        const { nameWrapper, baseRegistrar, ensRegistry, nameWrapperUpgraded, actions, accounts } = await loadFixture(
          fixture,
        );
      ...
      it("Will pass fuses and expiry to the upgradedContract without any changes.", async () => {
        ...
        await expect(nameWrapper)
          ...
          .withArgs(
            dnsEncodeName(name),
            accounts[0].address,
            PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CANNOT_SET_RESOLVER | IS_DOT_ETH,
            expectedExpiry,
            zeroAddress,
            "0x00",
          );
    ...
    describe("other", () => {
      ...
      it("Keeps approval information on upgrade", async () => {
        ...
        await expectOwnerOf(xyzName).on(nameWrapper).toBe(accounts[0]);
        await nameWrapper.write.approve([accounts[2].address, toNameId(xyzName)]);
