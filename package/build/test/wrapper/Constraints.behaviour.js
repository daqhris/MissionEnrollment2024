...
const { CANNOT_UNWRAP, CANNOT_SET_RESOLVER, PARENT_CANNOT_CONTROL, CAN_DO_EVERYTHING, IS_DOT_ETH } = FUSES;
...
async function baseFixture() {
  ...
  await initial.baseRegistrar.write.setApprovalForAll([initial.nameWrapper.address, true]);
  return initial;
}
// Reusable state setup
const setupState = ({ parentFuses, childFuses, childExpiry }) =>
  async function setupStateFixture() {
    ...
    await baseRegistrar.write.register([parentLabelId, accounts[0].address, DAY]);
    await nameWrapper.write.wrapETH2LD([parentLabel, accounts[0].address, parentFuses, zeroAddress]);
...
// Reusable state setup
const setupStateUnexpired = ({ parentFuses, childFuses }) =>
  async function setupStateUnexpiredFixture() {
    const initial = await loadFixture(baseFixture);
    const { baseRegistrar, nameWrapper, accounts } = initial;
    await baseRegistrar.write.register([parentLabelId, accounts[0].address, DAY * 2n]);
...
// Reusable tests
const parentCanExtend = ({ fixture, isNotExpired }) => {
  if (isNotExpired) {
    it("Child should have an expiry < parent", async () => {
      const { nameWrapper, baseRegistrar, publicClient } = await loadFixture(fixture);
  ...
  it("Parent can extend expiry with setChildFuses()", async () => {
    ...
    const parentExpiry = await baseRegistrar.read.nameExpires([parentLabelId]);
    await nameWrapper.write.setChildFuses([parentNode, childLabelHash, CAN_DO_EVERYTHING, MAX_EXPIRY]);
...
const parentCannotBurnFusesOrPCC = ({ fixture }) => {
  it("Parent cannot burn fuses with setChildFuses()", async () => {
    const { nameWrapper } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("setChildFuses", [parentNode, childLabelHash, CANNOT_UNWRAP | PARENT_CANNOT_CONTROL, 0n])
...
const parentCanReplaceOwner = ({ fixture }) => {
  it("Parent can replace owner with setSubnodeOwner()", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
    await nameWrapper.write.setSubnodeOwner([parentNode, childLabel, accounts[0].address, CAN_DO_EVERYTHING, 0n]);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[0].address);
  });
...
const parentCanUnwrapChild = ({ fixture }) => {
  it("Parent can unwrap owner with setSubnodeRecord() and then unwrap", async () => {
    ...
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
    await expect(ensRegistry.read.owner([childNode])).resolves.toEqualAddress(nameWrapper.address);
    await nameWrapper.write.setSubnodeRecord([
      parentNode,
      childLabel,
      accounts[0].address,
    ...
    await nameWrapper.write.unwrap([parentNode, childLabelHash, accounts[0].address]);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(zeroAddress);
    await expect(ensRegistry.read.owner([childNode])).resolves.toEqualAddress(accounts[0].address);
  });
};
const parentCannotBurnParentControlledFuses = ({ fixture }) => {
...
const ownerIsOwnerWhenExpired = ({ fixture }) => {
...
const ownerCannotBurnFuses = ({ fixture }) => {
...
const ownerCanUnwrap = ({ fixture }) => {
  it("Owner can unwrap", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await nameWrapper.write.unwrap([parentNode, childLabelHash, accounts[1].address], { account: accounts[1] });
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(zeroAddress);
  });
};
const parentCanBurnParentControlledFusesWithExpiry = ({ fixture }) => {
  it("Parent cannot burn parent-controlled fuses as they reset to 0", async () => {
    const { nameWrapper } = await loadFixture(fixture);
    await nameWrapper.write.setChildFuses([parentNode, childLabelHash, 1 << 18, 0n]);
  ...
  it("Parent can burn parent-controlled fuses, if expiry is extended", async () => {
    const { nameWrapper } = await loadFixture(fixture);
    await nameWrapper.write.setChildFuses([parentNode, childLabelHash, 1 << 18, MAX_EXPIRY]);
    const [, fuses] = await nameWrapper.read.getData([childNodeId]);
    expect(fuses).toEqual(1 << 18);
  });
};
const parentCanBurnFusesOrPCC = ({ fixture }) => {
  it("Parent can burn fuses with setChildFuses()", async () => {
    ...
    const [, fuses] = await nameWrapper.read.getData([childNodeId]);
    expect(fuses).toEqual(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER);
  });
  ...
  it("Parent cannot burn fuses if PCC is not burnt too", async () => {
    ...
    await expect(nameWrapper)
      ...
      .withArgs(childNode);
    await expect(nameWrapper)
      .write("setChildFuses", [parentNode, childLabelHash, CANNOT_UNWRAP | CANNOT_SET_RESOLVER, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, CANNOT_UNWRAP | CANNOT_SET_RESOLVER, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
  });
};
const parentCanBurnParentControlledFuses = ({ fixture }) => {
...
const testStateTransition1000to1010 = ({ fixture }) => {
  it("1000 => 1010 - Parent cannot burn PCC with setChildFuses()", async () => {
    const { nameWrapper } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("setChildFuses", [parentNode, childLabelHash, PARENT_CANNOT_CONTROL, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
  });
  it("1000 => 1010 - Parent cannot burn PCC with setSubnodeOwner()", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, PARENT_CANNOT_CONTROL, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
  });
...
const parentCanExtendWithSetChildFuses = ({ fixture }) => {
  ...
  it("Parent cannot extend expiry with setSubnodeOwner()", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, CAN_DO_EVERYTHING, MAX_EXPIRY])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
  });
...
const parentCannotBurnFusesWhenPCCisBurned = ({ fixture }) => {
  it("Parent cannot burn fuses with setChildFuses()", async () => {
    const { nameWrapper } = await loadFixture(fixture);
    ...
    await nameWrapper.write.setChildFuses([parentNode, childLabelHash, PARENT_CANNOT_CONTROL, 0n]);
    const [, fuses] = await nameWrapper.read.getData([childNodeId]);
  ...
  it("Parent cannot burn fuses with setSubnodeOwner()", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, CANNOT_SET_RESOLVER, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, CANNOT_UNWRAP | CANNOT_SET_RESOLVER, 0n])
...
const parentCannotReplaceOwner = ({ fixture }) => {
  it("Parent cannot replace owner with setSubnodeOwner()", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
    await expect(nameWrapper)
      .write("setSubnodeOwner", [parentNode, childLabel, accounts[0].address, CAN_DO_EVERYTHING, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
  });
  it("Parent cannot replace owner with setSubnodeRecord()", async () => {
    ...
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
    await expect(nameWrapper)
      .write("setSubnodeRecord", [parentNode, childLabel, accounts[0].address, zeroAddress, 0n, CAN_DO_EVERYTHING, 0n])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(childNode);
    await expect(nameWrapper.read.ownerOf([childNodeId])).resolves.toEqualAddress(accounts[1].address);
  });
};
const parentCannotUnwrapChild = ({ fixture }) => {
  it("Parent cannot unwrap itself", async () => {
    const { nameWrapper, accounts } = await loadFixture(fixture);
    await expect(nameWrapper)
      .write("unwrapETH2LD", [parentLabelHash, accounts[0].address, accounts[0].address])
      .toBeRevertedWithCustomError("OperationProhibited")
      .withArgs(parentNode);
  });
...
const ownerResetsToZeroWhenExpired = ({ fixture, expectedFuses }) => {
  it("Owner resets to 0 after expiry", async () => {
    const { nameWrapper, accounts, publicClient, testClient } = await loadFixture(fixture);
    const [ownerBefore, fusesBefore, expiryBefore] = await nameWrapper.read.getData([childNodeId]);
    const timestampBefore = await publicClient.getBlock().then(b => b.timestamp);
    // not expired
    ...
    const [ownerAfter, fusesAfter, expiryAfter] = await nameWrapper.read.getData([childNodeId]);
    const timestampAfter = await publicClient.getBlock().then(b => b.timestamp);
...
export const shouldRespectConstraints = () => {
  describe("0000 - Wrapped expired without CU/PCC burned, Parent's CU not burned", () => {
    ...
    it("correct test setup", async () => {
      ...
      const [, childFuses, childExpiry] = await nameWrapper.read.getData([childNodeId]);
  ...
  describe("0111 - CU_PCC_PCU - Impossible state - Wrapped expired, CU burned, PCC burned and Parent's CU burned", () => {
    ...
    it("0001 => 0111 - PCU => CU_PCC_PCU - Parent cannot burn PARENT_CANNOT_CONTROL | CANNOT_UNWRAP with setChildFuses()", async () => {
      const { nameWrapper } = await loadFixture(fixture);
      await nameWrapper.write.setChildFuses([parentNode, childLabelHash, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, 0n]);
      const [, fuses] = await nameWrapper.read.getData([childNodeId]);
      expect(fuses).toEqual(0);
    });
  });
  ...
  describe("1011 - NE_PCC_PCU Wrapped unexpired, CU, PCC and Parent's CU burned", () => {
    ...
    it("Parent cannot unburn fuses with setSubnodeOwner()", async () => {
      const { nameWrapper, accounts } = await loadFixture(fixture);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, 0, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(childNode);
      const [, fuses] = await nameWrapper.read.getData([childNodeId]);
      expect(fuses).toEqual(PARENT_CANNOT_CONTROL);
    ...
    it("Parent cannot unburn fuses with setSubnodeRecord()", async () => {
      const { nameWrapper, accounts } = await loadFixture(fixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [parentNode, childLabel, accounts[1].address, zeroAddress, 0n, 0, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(childNode);
      const [, fuses] = await nameWrapper.read.getData([childNodeId]);
      expect(fuses).toEqual(PARENT_CANNOT_CONTROL);
    ...
    it("Owner cannot unwrap and wrap to unburn PCC", async () => {
      ...
      await nameWrapper.write.wrap(
        [dnsEncodeName(`${childLabel}.${parentLabel}.eth`), accounts[1].address, zeroAddress],
        { account: accounts[1] },
      );
  ...
  describe("1100 - NE_CU - Impossible State - Wrapped unexpired, CU burned, and PCC and Parent's CU unburned ", () => {
    ...
    it("1000 => 1100 - NE => NE_CU - Parent cannot burn CU with setSubnodeOwner()", async () => {
      const { nameWrapper, accounts } = await loadFixture(fixture);
      await expect(nameWrapper)
        .write("setSubnodeOwner", [parentNode, childLabel, accounts[1].address, CANNOT_UNWRAP, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(childNode);
    });
    it("1000 => 1100 - NE => NE_CU - Parent cannot burn CU with setSubnodeRecord()", async () => {
      const { nameWrapper, accounts } = await loadFixture(fixture);
      await expect(nameWrapper)
        .write("setSubnodeRecord", [parentNode, childLabel, accounts[1].address, zeroAddress, 0n, CANNOT_UNWRAP, 0n])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(childNode);
    });
  ...
  describe("1111 - NE_CU_PCC_PCU - Wrapped unexpired, CU, PCC and Parent's CU burned ", () => {
    ...
    it("Owner can burn fuses", async () => {
      ...
      const [, fusesAfter] = await nameWrapper.read.getData([childNodeId]);
      expect(fusesAfter).toEqual(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | CANNOT_SET_RESOLVER);
    });
