...
const setupState = ({ parentFuses, childFuses, childExpiry, }) => async function setupStateFixture() {
...
// Reusable state setup
const setupStateUnexpired = ({ parentFuses, childFuses, }) => async function setupStateUnexpiredFixture() {
    const initial = await loadFixture(baseFixture);
    const { baseRegistrar, nameWrapper, accounts } = initial;
    await baseRegistrar.write.register([
        parentLabelId,
