...
describe("NameWrapper", () => {
  ...
  shouldBehaveLikeErc1155({
    ...
    targetTokenIds: [toNameId("test1.eth"), toNameId("test2.eth"), toNameId("doesnotexist.eth")],
    mint: async ({ accounts, actions }, [firstTokenHolder, secondTokenHolder]) => {
  ...
  describe("Transfer", () => {
    ...
    it("safeTransfer cannot be called if CANNOT_TRANSFER is burned and is not expired", async () => {
      ...
      await expect(nameWrapper)
        .write("safeTransferFrom", [accounts[0].address, accounts[1].address, toNameId(name), 1n, "0x"])
    ...
    it("safeBatchTransfer cannot be called if CANNOT_TRANSFER is burned and is not expired", async () => {
      ...
      await nameWrapper.write.setFuses([namehash(name), CANNOT_TRANSFER]);
      await expect(nameWrapper)
        .write("safeBatchTransferFrom", [accounts[0].address, accounts[1].address, [toNameId(name)], [1n], "0x"])
        .toBeRevertedWithCustomError("OperationProhibited")
        .withArgs(namehash(name));
    });
  });
  ...
  describe("MetadataService", () => {
    it("uri() returns url", async () => {
      ...
      await expect(nameWrapper.read.uri([123n])).resolves.toEqual("https://ens.domains");
    });
    it("owner can set a new MetadataService", async () => {
      const { nameWrapper, accounts } = await loadFixture(fixture);
      await nameWrapper.write.setMetadataService([accounts[1].address]);
      await expect(nameWrapper.read.metadataService()).resolves.toEqualAddress(accounts[1].address);
    });
  ...
  describe("NameWrapper.names preimage dictionary", () => {
    it("Does not allow manipulating the preimage db by manually setting owner as NameWrapper", async () => {
      const { baseRegistrar, ensRegistry, nameWrapper, accounts, testClient, publicClient, actions } =
      ...
      await expect(nameWrapper.read.names([namehash(subname2)])).resolves.toEqual(dnsEncodeName(subname2));
      ...
      await expect(nameWrapper.read.names([namehash(subname3)])).resolves.toEqual(dnsEncodeName(subname3));
    });
  });
  describe("Grace period tests", () => {
    ...
    async function gracePeriodFixture() {
      const initial = await loadFixture(fixture);
      const { nameWrapper, actions, accounts, testClient, publicClient } = initial;
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      const [, , parentExpiry] = await nameWrapper.read.getData([toNameId(name)]);
      ...
      const [, , subExpiry] = await nameWrapper.read.getData([toNameId(subname)]);
    ...
    it("When a .eth name is in grace period it cannot call setSubnodeOwner", async () => {
      const { nameWrapper, parentExpiry, accounts } = await loadFixture(gracePeriodFixture);
      await expect(nameWrapper)
    ...
    it("When a .eth name is in grace period it cannot call setRecord", async () => {
      ...
      await expect(nameWrapper)
        .write("setRecord", [namehash(name), accounts[1].address, zeroAddress, 0n])
        .toBeRevertedWithCustomError("Unauthorised")
        .withArgs(namehash(name), getAddress(accounts[0].address));
    });
    ...
    it("When a .eth name is in grace period, unexpired subdomains can set resolver", async () => {
      const { ensRegistry, nameWrapper, accounts } = await loadFixture(gracePeriodFixture);
      await nameWrapper.write.setResolver([namehash(subname), accounts[0].address], {
        account: accounts[1],
      });
      await expect(ensRegistry.read.resolver([namehash(subname)])).resolves.toEqualAddress(accounts[0].address);
    });
    it("When a .eth name is in grace period, unexpired subdomains can set ttl", async () => {
      ...
      await expect(ensRegistry.read.ttl([namehash(subname)])).resolves.toEqual(100n);
    });
    it("When a .eth name is in grace period, unexpired subdomains can call setRecord", async () => {
      const { ensRegistry, nameWrapper, accounts } = await loadFixture(gracePeriodFixture);
      await nameWrapper.write.setRecord([namehash(subname), accounts[0].address, accounts[1].address, 100n], {
        account: accounts[1],
      });
      await expectOwnerOf(subname).on(nameWrapper).toBe(accounts[0]);
      await expectOwnerOf(subname).on(ensRegistry).toBe(nameWrapper);
      await expect(ensRegistry.read.resolver([namehash(subname)])).resolves.toEqualAddress(accounts[1].address);
      await expect(ensRegistry.read.ttl([namehash(subname)])).resolves.toBe(100n);
    });
    it("When a .eth name is in grace period, unexpired subdomains can call setSubnodeOwner", async () => {
      const { nameWrapper, actions, accounts } = await loadFixture(gracePeriodFixture);
      await actions.setSubnodeOwner.onNameWrapper({
        parentName: subname,
        label: "sub2",
        owner: accounts[1].address,
    ...
    it("When a .eth name is in grace period, unexpired subdomains can call setChildFuses if the subdomain exists", async () => {
      ...
      await nameWrapper.write.setChildFuses([namehash(subname), labelhash("sub2"), 0, 100n], {
        account: accounts[1],
      });
      const [owner, fuses, expiry] = await nameWrapper.read.getData([toNameId(`sub2.${subname}`)]);
  ...
  describe("Registrar tests", () => {
    ...
    it("Reverts when attempting to call token owner protected function on an unwrapped name", async () => {
      const { ensRegistry, nameWrapper, baseRegistrar, actions, accounts, testClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      await expect(nameWrapper)
        .write(
          "setSubnodeOwner",
          [namehash(name), sublabel, accounts[1].address, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, MAX_EXPIRY],
          { account: accounts[2] },
        )
        .toBeRevertedWithCustomError("Unauthorised")
  ...
  describe("ERC1155 additional tests", () => {
    ...
    it("Transferring a token that is not owned by the owner reverts", async () => {
      ...
      await expect(nameWrapper)
        .write("safeTransferFrom", [accounts[2].address, accounts[0].address, toNameId(name), 1n, "0x"], {
          account: accounts[2],
        })
        .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
    });
    ...
    it("Approval on the Wrapper does not give permission to wrap a non .eth name", async () => {
      const { nameWrapper, ensRegistry, accounts, actions } = await loadFixture(fixture);
      await expectOwnerOf("xyz").on(ensRegistry).toBe(accounts[0]);
      await nameWrapper.write.setApprovalForAll([accounts[2].address, true]);
      await actions.setRegistryApprovalForWrapper();
      await expect(nameWrapper)
        .write("wrap", [dnsEncodeName("xyz"), accounts[2].address, zeroAddress], {
          account: accounts[2],
        })
        .toBeRevertedWithCustomError("Unauthorised")
    ...
    it("When .eth name expires, it is untransferrable", async () => {
      const { nameWrapper, actions, accounts, testClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CAN_DO_EVERYTHING,
      });
    ...
    it("Approval on the Wrapper does not give permission to transfer after expiry", async () => {
      ...
      await expect(nameWrapper)
        ...
        .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
      await expect(nameWrapper)
        .write("safeTransferFrom", [accounts[0].address, accounts[2].address, toNameId(name), 1n, "0x"], {
          account: accounts[2],
        })
        .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
    });
    it("When emancipated names expire, they are untransferrible", async () => {
      const { nameWrapper, actions, accounts, testClient, publicClient } = await loadFixture(fixture);
      await actions.registerSetupAndWrapName({
        label,
        fuses: CANNOT_UNWRAP,
      });
      ...
      await testClient.mine({ blocks: 1 });
      await expect(nameWrapper)
        .write("safeTransferFrom", [accounts[0].address, accounts[1].address, toNameId(`test.${name}`), 1n, "0x"])
        .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
    });
    it("Returns a balance of 0 for expired names", async () => {
      ...
      await expect(nameWrapper.read.balanceOf([accounts[0].address, toNameId(name)])).resolves.toEqual(1n);
      await testClient.increaseTime({ seconds: Number(86401n + GRACE_PERIOD) });
      await testClient.mine({ blocks: 1 });
      await expect(nameWrapper.read.balanceOf([accounts[0].address, toNameId(name)])).resolves.toEqual(0n);
    });
    it("Reregistering an expired name does not inherit its previous parent fuses", async () => {
      ...
      // Check PCC isn't set
      const [, fuses] = await nameWrapper.read.getData([toNameId(`test.${name}`)]);
      expect(fuses).toEqual(0);
    });
  });
  describe("Implicit unwrap tests", () => {
    ...
    it("Trying to burn child fuses when re-registering a name on the old controller reverts", async () => {
      const { ensRegistry, nameWrapper, baseRegistrar, actions, accounts, testClient } = await loadFixture(
        implicitUnwrapFixture,
      );
      await nameWrapper.write.registerAndWrapETH2LD([label, accounts[2].address, 1n * DAY, zeroAddress, CANNOT_UNWRAP]);
      ...
      await baseRegistrar.write.transferFrom([accounts[2].address, nameWrapper.address, toLabelId(label)], {
        account: accounts[2],
      });
      await expectOwnerOf(label).on(baseRegistrar).toBe(nameWrapper);
    ...
    it("Renewing a wrapped, but expired name .eth in the wrapper, but unexpired on the registrar resyncs expiry", async () => {
      const { ensRegistry, nameWrapper, baseRegistrar, accounts, testClient } = await loadFixture(
        implicitUnwrapFixture,
      );
      await nameWrapper.write.registerAndWrapETH2LD([label, accounts[0].address, 1n * DAY, zeroAddress, CANNOT_UNWRAP]);
      ...
      const registrarExpiry = await baseRegistrar.read.nameExpires([toLabelId(label)]);
  ...
  describe("TLD recovery", () => {
    it("Wraps a name which get stuck forever can be recovered by ROOT owner", async () => {
      const { ensRegistry, nameWrapper, accounts, actions } = await loadFixture(fixture);
      ...
      await nameWrapper.write.setChildFuses([zeroHash, labelhash("xyz"), PARENT_CANNOT_CONTROL, 0n]);
      await expectOwnerOf("xyz").on(nameWrapper).toBe(zeroAccount);
      await expectOwnerOf("xyz").on(ensRegistry).toBe(nameWrapper);
      await expect(nameWrapper)
        .write("setChildFuses", [zeroHash, labelhash("xyz"), PARENT_CANNOT_CONTROL, 100000000000000n])
        .toBeRevertedWithCustomError("NameIsNotWrapped");
      await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("xyz"), accounts[1].address]);
