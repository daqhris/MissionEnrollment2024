...
import { CANNOT_CREATE_SUBDOMAIN, CANNOT_TRANSFER, CANNOT_UNWRAP, CAN_DO_EVERYTHING, GRACE_PERIOD, MAX_EXPIRY, PARENT_CANNOT_CONTROL, expectOwnerOf, deployNameWrapperWithUtils as fixture, zeroAccount, } from './fixtures/utils.js';
...
describe('NameWrapper', () => {
    ...
    shouldBehaveLikeErc1155({
        contracts: () => loadFixture(fixture).then((contracts) => ({
    ...
    describe('NameWrapper.names preimage dictionary', () => {
        it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
            const { baseRegistrar, ensRegistry, nameWrapper, accounts, testClient, publicClient, actions, } = await loadFixture(fixture);
            const label = 'base';
            const name = `${label}.eth`;
            await actions.register({
                label,
    ...
    describe('Grace period tests', () => {
        ...
        it('When a .eth name is in grace period, unexpired subdomains can transfer', async () => {
            const { nameWrapper, accounts } = await loadFixture(gracePeriodFixture);
            await nameWrapper.write.safeTransferFrom([accounts[1].address, accounts[0].address, toNameId(subname), 1n, '0x'], { account: accounts[1] });
    ...
    describe('ERC1155 additional tests', () => {
        ...
        it('Transferring a token that is not owned by the owner reverts', async () => {
            ...
            await expect(nameWrapper)
                .write('safeTransferFrom', [accounts[2].address, accounts[0].address, toNameId(name), 1n, '0x'], { account: accounts[2] })
                .toBeRevertedWithString('ERC1155: insufficient balance for transfer');
        });
        ...
        it('Approval on the Wrapper does not give permission to transfer after expiry', async () => {
            ...
            await expect(nameWrapper)
                ...
                .toBeRevertedWithString('ERC1155: insufficient balance for transfer');
            await expect(nameWrapper)
                .write('safeTransferFrom', [accounts[0].address, accounts[2].address, toNameId(name), 1n, '0x'], { account: accounts[2] })
                .toBeRevertedWithString('ERC1155: insufficient balance for transfer');
        });
    ...
    describe('Implicit unwrap tests', () => {
        ...
        it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
            const { ensRegistry, nameWrapper, baseRegistrar, actions, accounts, testClient, } = await loadFixture(implicitUnwrapFixture);
            ...
            await baseRegistrar.write.transferFrom([accounts[2].address, nameWrapper.address, toLabelId(label)], { account: accounts[2] });
            await expectOwnerOf(label).on(baseRegistrar).toBe(nameWrapper);
        ...
        it('Renewing a wrapped, but expired name .eth in the wrapper, but unexpired on the registrar resyncs expiry', async () => {
            const { ensRegistry, nameWrapper, baseRegistrar, accounts, testClient } = await loadFixture(implicitUnwrapFixture);
            await nameWrapper.write.registerAndWrapETH2LD([
                label,
                accounts[0].address,
                1n * DAY,
