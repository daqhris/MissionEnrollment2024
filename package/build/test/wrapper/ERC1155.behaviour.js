...
const getNamedAccounts = ([minter, firstTokenHolder, secondTokenHolder, multiTokenHolder, recipient, proxy]) => ({
  minter,
  firstTokenHolder,
  secondTokenHolder,
  multiTokenHolder,
...
export const shouldBehaveLikeErc1155 = ({
  ...
  const contracts = async () => {
    ...
    return {
      ...
      ...contractsObject,
      mint: addresses => mint_(contractsObject, addresses),
      contract: contractsObject.contract,
    };
  };
  describe("like an ERC1155", () => {
    describe("balanceOf", () => {
      ...
      context("when accounts don't own tokens", () => {
        it("returns zero for given addresses", async () => {
          const { contract, firstTokenHolder, secondTokenHolder } = await contracts();
          await expect(contract.read.balanceOf([firstTokenHolder.address, firstTokenId])).resolves.toEqual(0n);
          await expect(contract.read.balanceOf([secondTokenHolder.address, secondTokenId])).resolves.toEqual(0n);
          await expect(contract.read.balanceOf([firstTokenHolder.address, unknownTokenId])).resolves.toEqual(0n);
        });
      });
      context("when accounts own some tokens", () => {
        it("returns the amount of tokens owned by the given addresses", async () => {
          const { contract, mint, firstTokenHolder, secondTokenHolder } = await contracts();
          await mint([firstTokenHolder.address, secondTokenHolder.address]);
          await expect(contract.read.balanceOf([firstTokenHolder.address, firstTokenId])).resolves.toEqual(1n);
          await expect(contract.read.balanceOf([secondTokenHolder.address, secondTokenId])).resolves.toEqual(1n);
          await expect(contract.read.balanceOf([firstTokenHolder.address, unknownTokenId])).resolves.toEqual(0n);
        });
      });
    });
    describe("balanceOfBatch", () => {
      it("reverts when input arrays don't match up", async () => {
        ...
        await expect(contract)
          .read("balanceOfBatch", [
            [firstTokenHolder.address, secondTokenHolder.address, firstTokenHolder.address, secondTokenHolder.address],
        ...
        it("returns zeros for each account", async () => {
          ...
          await expect(
            contract.read.balanceOfBatch([
              [firstTokenHolder.address, secondTokenHolder.address, firstTokenHolder.address],
              [firstTokenId, secondTokenId, unknownTokenId],
            ]),
      ...
      context("when accounts own some tokens", () => {
        it("returns amounts owned by each account in order passed", async () => {
          ...
          await expect(
            contract.read.balanceOfBatch([
              [secondTokenHolder.address, firstTokenHolder.address, firstTokenHolder.address],
    ...
    describe("setApprovalForAll", () => {
      it("sets approval status which can be queried via isApprovedForAll", async () => {
        ...
        await expect(contract.read.isApprovedForAll([multiTokenHolder.address, proxy.address])).resolves.toBe(true);
      });
      ...
      it("can unset approval for an operator", async () => {
        ...
        await expect(contract.read.isApprovedForAll([multiTokenHolder.address, proxy.address])).resolves.toBe(false);
      });
    ...
    async function mintedToMultiFixture() {
      ...
      await initial.mint([initial.multiTokenHolder.address, initial.multiTokenHolder.address]);
      return initial;
    }
    describe("safeTransferFrom", () => {
      it("reverts when transferring more than balance", async () => {
        const { contract, multiTokenHolder, recipient } = await loadFixture(mintedToMultiFixture);
        await expect(contract)
          .write("safeTransferFrom", [multiTokenHolder.address, recipient.address, firstTokenId, 2n, "0x"], {
            account: multiTokenHolder,
          })
          .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
      });
      it("reverts when transferring to zero address", async () => {
        const { contract, multiTokenHolder } = await loadFixture(mintedToMultiFixture);
        await expect(contract)
          .write("safeTransferFrom", [multiTokenHolder.address, zeroAddress, firstTokenId, 1n, "0x"], {
            account: multiTokenHolder,
          })
          .toBeRevertedWithString("ERC1155: transfer to the zero address");
      });
      const transferWasSuccessful = fixture => {
        it("debits transferred balance from sender", async () => {
          const { contract, from, id } = await loadFixture(fixture);
          await expect(contract.read.balanceOf([from.address, id])).resolves.toEqual(0n);
        });
        it("credits transferred balance to receiver", async () => {
          const { contract, to, id, value } = await loadFixture(fixture);
          await expect(contract.read.balanceOf([to.address, id])).resolves.toEqual(value);
        });
        it("emits a TransferSingle log", async () => {
          const { contract, operator, from, to, tx, id, value } = await loadFixture(fixture);
          await expect(contract)
            .transaction(tx)
      ...
      context("when called by the multiTokenHolder", () => {
        async function fixture() {
          ...
          const value = 1n;
          const tx = await contract.write.safeTransferFrom([from.address, to.address, id, value, "0x"], {
            account: operator,
          });
        ...
        it("preserves existing balances which are not transferred by multiTokenHolder", async () => {
          const { contract, multiTokenHolder, recipient } = await loadFixture(fixture);
          await expect(contract.read.balanceOf([multiTokenHolder.address, secondTokenId])).resolves.toEqual(1n);
          await expect(contract.read.balanceOf([recipient.address, secondTokenId])).resolves.toEqual(0n);
        });
      });
      context("when called by an operator on behalf of the multiTokenHolder", () => {
        context("when operator is not approved by multiTokenHolder", () => {
          it("reverts", async () => {
            const { contract, multiTokenHolder, recipient, proxy } = await loadFixture(mintedToMultiFixture);
            await expect(contract)
              .write("safeTransferFrom", [multiTokenHolder.address, recipient.address, firstTokenId, 1n, "0x"], {
                account: proxy,
              })
        ...
        context("when operator is approved by multiTokenHolder", () => {
          async function fixture() {
            const contractsObject = await loadFixture(mintedToMultiFixture);
            const { contract, multiTokenHolder, proxy, recipient } = contractsObject;
          ...
          it("preserves operator's balances not involved in the transfer", async () => {
            ...
            await expect(contract.read.balanceOf([proxy.address, firstTokenId])).resolves.toEqual(0n);
            await expect(contract.read.balanceOf([proxy.address, secondTokenId])).resolves.toEqual(0n);
          });
        });
      });
      context("when sending to a valid receiver", () => {
        const createValidReceiverFixture = data =>
          async function contractsWithReceiver() {
            const contractsObject = await loadFixture(mintedToMultiFixture);
            const receiver = await hre.viem.deployContract("ERC1155ReceiverMock", [
            ...
            const id = firstTokenId;
            const value = 1n;
            const tx = await contract.write.safeTransferFrom([from.address, to.address, id, value, data], {
              account: operator,
            });
            return {
              ...contractsObject,
        ...
        context("without data", () => {
          ...
          it("calls onERC1155Received", async () => {
            const { contract, receiver, multiTokenHolder, tx } = await loadFixture(fixture);
            await expect(contract)
              .transaction(tx)
              .toEmitEventFrom(receiver, "Received")
              .withArgs(multiTokenHolder.address, multiTokenHolder.address, firstTokenId, 1n, "0x");
          });
        });
        context("with data", () => {
          ...
          it("calls onERC1155Received", async () => {
            ...
            await expect(contract)
              .transaction(tx)
              .toEmitEventFrom(receiver, "Received")
              .withArgs(multiTokenHolder.address, multiTokenHolder.address, firstTokenId, 1n, data);
          });
        });
      });
      context("to a receiver contract returning unexpected value", () => {
        it("reverts", async () => {
          ...
          await expect(contract)
            .write("safeTransferFrom", [multiTokenHolder.address, receiver.address, firstTokenId, 1n, "0x"], {
              account: multiTokenHolder,
            })
      ...
      context("to a contract that does not implement the required function", () => {
        it("reverts", async () => {
          ...
          await expect(contract)
            ...
            .toBeRevertedWithString("ERC1155: transfer to non ERC1155Receiver implementer");
        });
      });
    });
    describe("safeBatchTransferFrom", () => {
      it("reverts when transferring amount more than any of balances", async () => {
        ...
        await expect(contract)
          .write(
            ...
            [multiTokenHolder.address, recipient.address, [firstTokenId, secondTokenId], [1n, 2n], "0x"],
            { account: multiTokenHolder },
          )
          .toBeRevertedWithString("ERC1155: insufficient balance for transfer");
      });
      it("reverts when ids array length doesn't match amounts array length", async () => {
        ...
        await expect(contract)
          .write(
            "safeBatchTransferFrom",
            [multiTokenHolder.address, recipient.address, [firstTokenId], [1n, 1n], "0x"],
            { account: multiTokenHolder },
          )
        ...
        await expect(contract)
          .write(
            "safeBatchTransferFrom",
            [multiTokenHolder.address, recipient.address, [firstTokenId, secondTokenId], [1n], "0x"],
            { account: multiTokenHolder },
          )
          .toBeRevertedWithString("ERC1155: ids and amounts length mismatch");
      });
      it("reverts when transferring to zero address", async () => {
        ...
        await expect(contract)
          .write(
            "safeBatchTransferFrom",
            [multiTokenHolder.address, zeroAddress, [firstTokenId, secondTokenId], [1n, 1n], "0x"],
            { account: multiTokenHolder },
          )
          .toBeRevertedWithString("ERC1155: transfer to the zero address");
      });
      const batchTransferWasSuccessful = fixture => {
        it("debits transferred balance from sender", async () => {
          const { contract, from, ids } = await loadFixture(fixture);
          await expect(contract.read.balanceOfBatch([new Array(ids.length).fill(from.address), ids])).resolves.toEqual(
            new Array(ids.length).fill(0n),
          );
        });
        it("credits transferred balance to receiver", async () => {
          const { contract, to, ids, values } = await loadFixture(fixture);
          await expect(contract.read.balanceOfBatch([new Array(ids.length).fill(to.address), ids])).resolves.toEqual(
            values,
          );
        });
        it("emits a TransferSingle log", async () => {
          const { contract, operator, from, to, tx, ids, values } = await loadFixture(fixture);
          await expect(contract)
            .transaction(tx)
      ...
      context("when called by the multiTokenHolder", () => {
        async function fixture() {
          ...
          const values = [1n, 1n];
          const tx = await contract.write.safeBatchTransferFrom([from.address, to.address, ids, values, "0x"], {
            account: operator,
          });
        ...
        context("when operator is not approved by multiTokenHolder", () => {
          it("reverts", async () => {
            ...
            await expect(contract)
              .write(
                "safeBatchTransferFrom",
                [multiTokenHolder.address, recipient.address, [firstTokenId, secondTokenId], [1n, 1n], "0x"],
                { account: proxy },
              )
              .toBeRevertedWithString("ERC1155: transfer caller is not owner nor approved");
          });
        });
        context("when operator is approved by multiTokenHolder", () => {
          async function fixture() {
      ...
      context("when sending to a valid receiver", () => {
        const createValidReceiverFixture = data =>
          async function contractsWithReceiver() {
            ...
            const ids = [firstTokenId, secondTokenId];
            const values = [1n, 1n];
            const tx = await contract.write.safeBatchTransferFrom([from.address, to.address, ids, values, data], {
              account: operator,
            });
            return {
              ...contractsObject,
      ...
      context("to a receiver contract returning unexpected value", () => {
        it("reverts", async () => {
          ...
          await expect(contract)
            .write(
              "safeBatchTransferFrom",
              [multiTokenHolder.address, receiver.address, [firstTokenId, secondTokenId], [1n, 1n], "0x"],
              { account: multiTokenHolder },
            )
            .toBeRevertedWithString("ERC1155: ERC1155Receiver rejected tokens");
        });
      ...
      context("to a receiver contract that reverts", () => {
        it("reverts", async () => {
          ...
          await expect(contract)
            ...
            .toBeRevertedWithString("ERC1155ReceiverMock: reverting on batch receive");
        });
      });
