...
async function fixture() {
  ...
  await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("reverse"), accounts[0].address]);
  await ensRegistry.write.setSubnodeOwner([namehash("reverse"), labelhash("addr"), reverseRegistrar.address]);
  ...
  await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("eth"), accounts[0].address]);
  return {
    ensRegistry,
    nameWrapper,
    reverseRegistrar,
...
async function fixtureWithDnsRecords() {
  ...
  const hash = await existing.publicResolver.write.setDNSRecords([targetNode, rec]);
...
describe("PublicResolver", () => {
  ...
  describe("supportsInterface function", () => {
    it("supports known interfaces", async () => {
      ...
      const interfaceIds = expectedArtifactSupport.map(a => createInterfaceId(a.abi));
      for (const interfaceId of interfaceIds) {
        await expect(publicResolver.read.supportsInterface([interfaceId])).resolves.toEqual(true);
      }
    });
    it("does not support a random interface", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver.read.supportsInterface(["0x3b3b57df"])).resolves.toEqual(false);
    });
  });
  ...
  describe("addr", () => {
    it("permits setting address by owner", async () => {
      ...
      const hash = await publicResolver.write.setAddr([targetNode, accounts[1].address]);
      await expect(publicResolver)
        .transaction(hash)
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(accounts[1].address);
    });
    it("can overwrite previously set address", async () => {
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(accounts[0].address);
    });
    ...
    it("returns zero when fetching nonexistent addresses", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(zeroAddress);
    });
    it("permits setting and retrieving addresses for other coin types", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.setAddr([targetNode, 123n, accounts[1].address]);
      await expect(publicResolver.read.addr([targetNode, 123n])).resolves.toEqual(accounts[1].address.toLowerCase());
    });
    it("returns ETH address for coin type 60", async () => {
      ...
      await expect(publicResolver)
        ...
        .toEmitEvent("AddrChanged")
        .withArgs(targetNode, accounts[1].address);
      await expect(publicResolver.read.addr([targetNode, 60n])).resolves.toEqual(accounts[1].address.toLowerCase());
    });
    it("setting coin type 60 updates ETH address", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      const hash = await publicResolver.write.setAddr([targetNode, 60n, accounts[2].address]);
      await expect(publicResolver)
        .transaction(hash)
        .toEmitEvent("AddressChanged")
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(accounts[2].address);
    });
  ...
  describe("name", () => {
    it("permits setting name by owner", async () => {
      ...
      await expect(publicResolver.read.name([targetNode])).resolves.toEqual("name1");
    });
    it("can overwrite previously set names", async () => {
      ...
      await expect(publicResolver.read.name([targetNode])).resolves.toEqual("name2");
    });
  ...
  describe("pubkey", async () => {
    const pubkeyEmpty = [zeroHash, zeroHash];
    const pubkey1 = [padHex("0x10", { dir: "right", size: 32 }), padHex("0x20", { dir: "right", size: 32 })];
    const pubkey2 = [padHex("0x30", { dir: "right", size: 32 }), padHex("0x40", { dir: "right", size: 32 })];
    it("returns empty when fetching nonexistent values", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver.read.pubkey([targetNode])).resolves.toMatchObject(pubkeyEmpty);
    });
    it("permits setting public key by owner", async () => {
      ...
      await expect(publicResolver.read.pubkey([targetNode])).resolves.toMatchObject(pubkey1);
    });
    it("can overwrite previously set value", async () => {
      ...
      await expect(publicResolver.read.pubkey([targetNode])).resolves.toMatchObject(pubkey2);
    });
  ...
  describe("ABI", () => {
    it("returns a contentType of 0 when nothing is available", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver.read.ABI([targetNode, 0xffffffffn])).resolves.toMatchObject([0n, "0x"]);
    });
    it("returns an ABI after it has been set", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await publicResolver.write.setABI([targetNode, 1n, "0x666f6f"]);
      await expect(publicResolver.read.ABI([targetNode, 0xffffffffn])).resolves.toMatchObject([1n, "0x666f6f"]);
    });
    it("returns the first valid ABI", async () => {
      ...
      await expect(publicResolver.read.ABI([targetNode, 0x7n])).resolves.toMatchObject([2n, "0x666f6f"]);
      await expect(publicResolver.read.ABI([targetNode, 0x5n])).resolves.toMatchObject([4n, "0x626172"]);
    });
    ...
    it("rejects invalid content types", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver).write("setABI", [targetNode, 0x3n, "0x12"]).toBeRevertedWithoutReason();
    });
    it("forbids setting value by non-owners", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await expect(publicResolver)
  ...
  describe("test", () => {
    ...
    it("permits setting text by owner", async () => {
      ...
      await expect(publicResolver.read.text([targetNode, "url"])).resolves.toEqual(url1);
    });
    it("can overwrite previously set text", async () => {
      ...
      await expect(publicResolver.read.text([targetNode, "url"])).resolves.toEqual(url2);
    });
    ...
    it("resets record on version change", async () => {
      ...
      await expect(publicResolver.read.text([targetNode, "url"])).resolves.toEqual(url1);
      await publicResolver.write.clearRecords([targetNode]);
      await expect(publicResolver.read.text([targetNode, "url"])).resolves.toEqual("");
    });
  });
  describe("contenthash", () => {
    ...
    it("permits setting contenthash by owner", async () => {
      ...
      await expect(publicResolver.read.contenthash([targetNode])).resolves.toEqual(contenthash1);
    });
    it("can overwrite previously set contenthash", async () => {
      ...
      await expect(publicResolver.read.contenthash([targetNode])).resolves.toEqual(contenthash2);
    });
    ...
    it("returns empty when fetching nonexistent contenthash", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await expect(publicResolver.read.contenthash([targetNode])).resolves.toEqual("0x");
    });
    it("resets record on version change", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await publicResolver.write.setContenthash([targetNode, contenthash1]);
  ...
  describe("dns", () => {
    describe("records", () => {
      it("permits setting name by owner", async () => {
        const { publicResolver, hash, arec, b1rec, b2rec, soarec } = await loadFixture(fixtureWithDnsRecords);
        await expect(publicResolver).transaction(hash).toEmitEvent("DNSRecordChanged");
        await expect(
          publicResolver.read.dnsRecord([targetNode, keccak256(dnsEncodeName("a.eth.")), 1]),
        ).resolves.toEqual(`0x${arec}`);
        await expect(
          publicResolver.read.dnsRecord([targetNode, keccak256(dnsEncodeName("b.eth.")), 1]),
        ).resolves.toEqual(`0x${b1rec}${b2rec}`);
        await expect(publicResolver.read.dnsRecord([targetNode, keccak256(dnsEncodeName("eth.")), 6])).resolves.toEqual(
          `0x${soarec}`,
        );
      });
      ...
      it("should keep track of entries", async () => {
        ...
        // Initial check
        await expect(
          publicResolver.read.hasDNSRecords([targetNode, keccak256(dnsEncodeName("c.eth."))]),
        ).resolves.toEqual(true);
        await expect(
          publicResolver.read.hasDNSRecords([targetNode, keccak256(dnsEncodeName("d.eth."))]),
      ...
      it("should handle single-record updates", async () => {
        ...
        await publicResolver.write.setDNSRecords([targetNode, rec]);
        await expect(
          publicResolver.read.dnsRecord([targetNode, keccak256(dnsEncodeName("e.eth.")), 1]),
        ).resolves.toEqual(`0x${erec}`);
      });
      it("forbids setting DNS records by non-owners", async () => {
        const { publicResolver, accounts } = await loadFixture(fixtureWithDnsRecords);
    ...
    describe("zonehash", () => {
      ...
      it("permits setting zonehash by owner", async () => {
        const { publicResolver } = await loadFixture(fixture);
        await expect(publicResolver).write("setZonehash", [targetNode, zonehash1]).toEmitEvent("DNSZonehashChanged");
        await expect(publicResolver.read.zonehash([targetNode])).resolves.toEqual(zonehash1);
      });
      it("can overwrite previously set zonehash", async () => {
        ...
        await expect(publicResolver.read.zonehash([targetNode])).resolves.toEqual(zonehash2);
      });
      ...
      it("returns empty when fetching nonexistent zonehash", async () => {
        const { publicResolver } = await loadFixture(fixture);
        await expect(publicResolver.read.zonehash([targetNode])).resolves.toEqual("0x");
      });
  ...
  describe("implementsInterface", () => {
    ...
    it("permits setting interface by owner", async () => {
      ...
      await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(
        accounts[0].address,
      );
    });
    it("can update previously set interface", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.setInterface([targetNode, interface1, accounts[0].address]);
      await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(
        accounts[0].address,
      );
      await publicResolver.write.setInterface([targetNode, interface1, accounts[1].address]);
      await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(
        accounts[1].address,
      );
    });
    ...
    it("falls back to calling implementsInterface on addr", async () => {
      ...
      await expect(publicResolver.read.interfaceImplementer([targetNode, addrInterfaceId])).resolves.toEqualAddress(
    ...
    it("returns 0 on fallback when target contract does not support implementsInterface", async () => {
      ...
      const supportsInterfaceArtifact = await hre.artifacts.readArtifact("IERC165");
      const supportsInterfaceId = createInterfaceId(supportsInterfaceArtifact.abi);
      await expect(publicResolver.read.interfaceImplementer([targetNode, supportsInterfaceId])).resolves.toEqualAddress(
        zeroAddress,
      );
    });
    ...
    it("resets record on version change", async () => {
      const { publicResolver } = await loadFixture(fixture);
      await publicResolver.write.setInterface([targetNode, interface1, publicResolver.address]);
      await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(
        publicResolver.address,
      );
      await publicResolver.write.clearRecords([targetNode]);
  ...
  describe("authorisations", () => {
    it("permits authorisations to be set", async () => {
      ...
      await expect(publicResolver.read.isApprovedForAll([accounts[0].address, accounts[1].address])).resolves.toEqual(
        true,
      );
    });
    ...
    it("permits non-owners to set authorisations", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.setApprovalForAll([accounts[2].address, true], {
        account: accounts[1],
      });
    ...
    it("checks the authorisation for the current owner", async () => {
      const { ensRegistry, publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.setApprovalForAll([accounts[2].address, true], { account: accounts[1] });
    ...
    it("trusted contract can bypass authorisation", async () => {
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(accounts[9].address);
    });
    ...
    it("permits name wrapper owner to make changes if owner is set to name wrapper address", async () => {
      const { ensRegistry, nameWrapper, publicResolver, accounts } = await loadFixture(fixture);
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(owner.address);
    });
  });
  describe("token approvals", async () => {
    it("permits delegate to be approved", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.approve([targetNode, accounts[1].address, true]);
      await expect(
        publicResolver.read.isApprovedFor([accounts[0].address, targetNode, accounts[1].address]),
      ).resolves.toEqual(true);
    });
    ...
    it("permits delegations to be cleared", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.approve([targetNode, accounts[1].address, true]);
      await publicResolver.write.approve([targetNode, accounts[1].address, false]);
      await expect(publicResolver)
        .write("setAddr", [targetNode, accounts[0].address], {
          account: accounts[1],
        })
    ...
    it("permits non-owners to set delegations", async () => {
      const { publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.approve([targetNode, accounts[2].address, true], {
        account: accounts[1],
      });
    ...
    it("checks the delegation for the current owner", async () => {
      const { ensRegistry, publicResolver, accounts } = await loadFixture(fixture);
      await publicResolver.write.approve([targetNode, accounts[2].address, true], { account: accounts[1] });
      await ensRegistry.write.setOwner([targetNode, accounts[1].address]);
      await publicResolver.write.setAddr([targetNode, accounts[0].address], {
        account: accounts[2],
      });
  ...
  describe("multicall", async () => {
    ...
    it("allows setting multiple fields", async () => {
      ...
      const hash = await publicResolver.write.multicall([[setAddrCall, setTextCall]]);
      await expect(publicResolver)
        .transaction(hash)
        .toEmitEvent("AddrChanged")
        .withArgs(targetNode, accounts[1].address);
      ...
      await expect(publicResolver.read.addr([targetNode])).resolves.toEqualAddress(accounts[1].address);
      await expect(publicResolver.read.text([targetNode, "url"])).resolves.toEqual(urlValue);
    });
