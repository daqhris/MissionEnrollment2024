...
import { decodeFunctionResult, encodeFunctionData, keccak256, labelhash, namehash, padHex, zeroAddress, zeroHash, } from 'viem';
...
describe('PublicResolver', () => {
    describe('fallback function', () => {
        it('forbids calls to the fallback function with 0 value', async () => {
            ...
            await expect(publicResolver)
                .transaction(walletClients[0].sendTransaction({
    ...
    describe('dns', () => {
        describe('records', () => {
            it('permits setting name by owner', async () => {
                ...
                await expect(publicResolver.read.dnsRecord([
                    targetNode,
            ...
            it('should keep track of entries', async () => {
                ...
                await publicResolver.write.setDNSRecords([targetNode, rec]);
                // Initial check
                await expect(publicResolver.read.hasDNSRecords([
                    targetNode,
    ...
    describe('implementsInterface', () => {
        ...
        it('permits setting interface by owner', async () => {
            ...
            await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(accounts[0].address);
        });
        it('can update previously set interface', async () => {
            ...
            await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(accounts[1].address);
        });
        ...
        it('returns zero when fetching nonexistent interface', async () => {
            const { publicResolver } = await loadFixture(fixture);
            await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(zeroAddress);
        });
        it('falls back to calling implementsInterface on addr', async () => {
            ...
            await expect(publicResolver.read.interfaceImplementer([targetNode, addrInterfaceId])).resolves.toEqualAddress(publicResolver.address);
        });
        ...
        it('returns 0 on fallback when target contract does not support implementsInterface', async () => {
            ...
            await expect(publicResolver.read.interfaceImplementer([
                targetNode,
                supportsInterfaceId,
        ...
        it('resets record on version change', async () => {
            ...
            await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(publicResolver.address);
            await publicResolver.write.clearRecords([targetNode]);
            await expect(publicResolver.read.interfaceImplementer([targetNode, interface1])).resolves.toEqualAddress(zeroAddress);
        });
    });
    describe('authorisations', () => {
        it('permits authorisations to be set', async () => {
            ...
            await expect(publicResolver.read.isApprovedForAll([
                accounts[0].address,
                accounts[1].address,
            ])).resolves.toEqual(true);
        });
    ...
    describe('token approvals', async () => {
        it('permits delegate to be approved', async () => {
            ...
            await expect(publicResolver.read.isApprovedFor([
                accounts[0].address,
                targetNode,
                accounts[1].address,
            ])).resolves.toEqual(true);
    ...
    describe('multicall', async () => {
        ...
        it('allows reading multiple fields', async () => {
            ...
            const { result: [addrResult, textResult], } = await publicResolver.simulate.multicall([[addrCall, textCall]]);
