...
digests.forEach((testcase) => {
    ...
    describe(testcase.digest, () => {
        it('should return true for valid hashes', async () => {
            ...
            await Promise.all(testcase.valids.map(async ([text, hash]) => expect(digest.read.verify([stringToHex(text), hash])).resolves.toBe(true)));
        });
        it('should return false for invalid hashes', async () => {
            const { digest } = await fixture();
            await Promise.all(testcase.invalids.map(async ([text, hash]) => expect(digest.read.verify([stringToHex(text), hash])).resolves.toBe(false)));
        });
        it('should throw an error for hashes of the wrong form', async () => {
            ...
            await Promise.all(testcase.errors.map(async ([text, hash]) => expect(digest)
