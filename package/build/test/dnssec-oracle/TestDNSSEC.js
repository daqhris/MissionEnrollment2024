import { expiration, hexEncodeSignedSet, inception, rootKeys } from "../fixtures/dns.js";
...
describe("DNSSEC", () => {
  it("should accept real DNSSEC records", async () => {
    ...
    const [rrs] = await dnssec.read.verifyRRSet([sets, TEST_RRSET_TIMESTAMP]);
    const [, data, sig] = test_rrsets[test_rrsets.length - 1];
    const expected = SignedSet.fromWire(Buffer.from(data.slice(2), "hex"), Buffer.from(sig.slice(2), "hex"));
    expect(rrs.slice(2)).toEqual(expected.toWire(false).toString("hex"));
  });
  it("should have a default algorithm and digest set", async () => {
    const { dnssec } = await loadFixture(dnssecFixture);
    await expect(dnssec.read.algorithms([8])).not.resolves.toEqualAddress(zeroAddress);
    await expect(dnssec.read.algorithms([253])).not.resolves.toEqualAddress(zeroAddress);
    await expect(dnssec.read.digests([2])).not.resolves.toEqualAddress(zeroAddress);
    await expect(dnssec.read.digests([253])).not.resolves.toEqualAddress(zeroAddress);
  });
  ...
  it("should reject signatures with non-IN classes", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("InvalidClass");
  });
  it("should reject signatures with the wrong type covered", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("SignatureTypeMismatch");
  });
  it("should reject signatures with too many labels", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("InvalidLabelCount");
  });
  it("should reject signatures with invalid signer names", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("InvalidSignerName");
  });
  ...
  it("should reject signatures with unknown algorithms", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("NoMatchingProof");
  });
        ...
        [
          {
            ...
            sig: `${sig.slice(0, sig.length - 2)}FF`,
          },
        ],
  ...
  it("should reject DS proofs with the wrong name", async () => {
    ...
    await expect(dnssec).read("verifyRRSet", [set]).toBeRevertedWithCustomError("ProofNameMismatch");
  });
