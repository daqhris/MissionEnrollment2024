...
describe('ReverseRegistrar', () => {
    it('should calculate node hash correctly', async () => {
        ...
        await expect(reverseRegistrar.read.node([accounts[0].address])).resolves.toEqual(getReverseNodeHash(accounts[0].address));
    });
    describe('claim', () => {
        it('allows an account to claim its address', async () => {
            ...
            await expect(ensRegistry.read.owner([getReverseNodeHash(accounts[0].address)])).resolves.toEqualAddress(accounts[1].address);
        });
    ...
    describe('claimForAddr', () => {
        ...
        it('allows an authorised account to claim a different address', async () => {
            ...
            await expect(ensRegistry.read.owner([getReverseNodeHash(accounts[1].address)])).resolves.toEqualAddress(accounts[2].address);
        });
        ...
        it('allows an owner() of a contract to claim the reverse node of that contract', async () => {
            ...
            await expect(ensRegistry.read.owner([getReverseNodeHash(dummyOwnable.address)])).resolves.toEqualAddress(accounts[0].address);
        });
    });
    describe('claimWithResolver', async () => {
        it('allows an account to specify resolver', async () => {
            ...
            await expect(ensRegistry.read.owner([getReverseNodeHash(accounts[0].address)])).resolves.toEqualAddress(accounts[1].address);
            await expect(ensRegistry.read.resolver([getReverseNodeHash(accounts[0].address)])).resolves.toEqualAddress(accounts[2].address);
        });
        it('event ReverseClaimed is emitted', async () => {
            const { reverseRegistrar, accounts } = await loadFixture(fixture);
            await expect(reverseRegistrar)
    ...
    describe('setName', () => {
        it('allows controller to set name records for other accounts', async () => {
            ...
            await expect(ensRegistry.read.resolver([getReverseNodeHash(accounts[1].address)])).resolves.toEqualAddress(publicResolver.address);
        ...
        it('allows name to be set for an address if the sender is the address', async () => {
            ...
            await expect(ensRegistry.read.resolver([getReverseNodeHash(accounts[0].address)])).resolves.toEqualAddress(publicResolver.address);
        ...
        it('allows an owner() of a contract to claimWithResolverForAddr on behalf of the contract', async () => {
            ...
            await expect(ensRegistry.read.owner([getReverseNodeHash(dummyOwnable.address)])).resolves.toEqualAddress(accounts[0].address);
            await expect(publicResolver.read.name([getReverseNodeHash(dummyOwnable.address)])).resolves.toEqual('dummyownable.eth');
        });
    });
