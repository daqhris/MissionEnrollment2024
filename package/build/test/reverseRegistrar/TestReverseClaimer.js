...
describe('ReverseClaimer', () => {
    it('claims a reverse node to the msg.sender of the deployer', async () => {
        ...
        await expect(ensRegistry.read.owner([namehash(getReverseNode(nameWrapper.address))])).resolves.toEqualAddress(accounts[0].address);
    });
    it('claims a reverse node to an address specified by the deployer', async () => {
        ...
        const mockReverseClaimerImplementer = await hre.viem.deployContract('MockReverseClaimerImplementer', [ensRegistry.address, accounts[1].address]);
