...
describe("ENSRegistry", () => {
  it("should allow ownership transfers", async () => {
    ...
    await expect(ensRegistry.read.owner([zeroHash])).resolves.toEqual(placeholderAddr);
  });
  ...
  it("should allow setting resolvers", async () => {
    ...
    await expect(ensRegistry)
      ...
      .withArgs(zeroHash, placeholderAddr);
    await expect(ensRegistry.read.resolver([zeroHash])).resolves.toEqual(placeholderAddr);
  });
  ...
  it("should allow setting the TTL", async () => {
    const { ensRegistry } = await loadFixture(fixture);
    await expect(ensRegistry).write("setTTL", [zeroHash, 3600n]).toEmitEvent("NewTTL").withArgs(zeroHash, 3600n);
  ...
  it("should allow the creation of subnodes", async () => {
    ...
    await expect(ensRegistry)
      .write("setSubnodeOwner", [zeroHash, labelhash("eth"), accounts[1].address])
  ...
  it("should prohibit subnode creation by non-owners", async () => {
    const { ensRegistry, accounts } = await loadFixture(fixture);
    await expect(ensRegistry)
      .write("setSubnodeOwner", [zeroHash, labelhash("eth"), accounts[1].address], { account: accounts[1] })
      .toBeRevertedWithoutReason();
  });
});
