...
async function fixture() {
  ...
  const fifsRegistrar = await hre.viem.deployContract("FIFSRegistrar", [ensRegistry.address, zeroHash]);
...
async function fixtureWithEthSet() {
  ...
  await existing.fifsRegistrar.write.register([labelhash("eth"), existing.accounts[0].address]);
  return existing;
}
describe("FIFSRegistrar", () => {
  it("should allow registration of names", async () => {
    ...
    await expect(ensRegistry.read.owner([zeroHash])).resolves.toEqualAddress(fifsRegistrar.address);
    await expect(ensRegistry.read.owner([namehash("eth")])).resolves.toEqualAddress(accounts[0].address);
  });
  describe("transferring names", () => {
    it("should allow transferring name to your own", async () => {
      const { fifsRegistrar, ensRegistry, accounts } = await loadFixture(fixtureWithEthSet);
      await fifsRegistrar.write.register([labelhash("eth"), accounts[1].address]);
      await expect(ensRegistry.read.owner([namehash("eth")])).resolves.toEqualAddress(accounts[1].address);
    });
