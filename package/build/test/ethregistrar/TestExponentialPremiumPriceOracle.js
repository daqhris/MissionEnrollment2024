...
describe('ExponentialPremiumPriceOracle', () => {
    ...
    it('should specify the correct price after 2.5 days and 1 year registration', async () => {
        ...
        await expect(priceOracle.read
            .premium(['foobar', timestamp, lengthOfRegistration])
            .then((p) => (Number(p) / 1e18).toPrecision(15))).resolves.toEqual(expectedPremium);
        await expect(priceOracle.read
            .price(['foobar', timestamp, lengthOfRegistration])
            .then((p) => (Number(p.premium) / 1e18).toPrecision(15))).resolves.toEqual(expectedPremium);
    });
    it('should produce a 0 premium at the end of the decay period', async () => {
        ...
        await expect(priceOracle.read.premium(['foobar', timestamp - LAST_DAY * DAY + 1n, 0n])).resolves.toBeGreaterThan(0n);
    ...
    it('should not be beyond a certain amount of inaccuracy from floating point calc', async () => {
        ...
        const result = await Promise.all(Array.from({ length: Number(DAY * LAST_DAY) / interval }).map(async (_, i) => {
            ...
            if (contractResult === 0)
                return { percent: 0, absoluteDifference: 0 };
            ...
            if (curr.percent > prev.percentMax &&
