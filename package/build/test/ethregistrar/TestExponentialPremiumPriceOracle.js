...
async function fixture() {
  ...
  const priceOracle = await hre.viem.deployContract("ExponentialPremiumPriceOracle", [
    dummyOracle.address,
    [0n, 0n, 4n, 2n, 1n],
...
describe("ExponentialPremiumPriceOracle", () => {
  it("should return correct base prices", async () => {
    ...
    await expect(priceOracle.read.price(["foo", 0n, 3600n])).resolves.toHaveProperty("base", 7200n);
    await expect(priceOracle.read.price(["quux", 0n, 3600n])).resolves.toHaveProperty("base", 3600n);
    await expect(priceOracle.read.price(["fubar", 0n, 3600n])).resolves.toHaveProperty("base", 1800n);
    await expect(priceOracle.read.price(["foobie", 0n, 3600n])).resolves.toHaveProperty("base", 1800n);
  });
  it("should not specify a premium for first-time registrations", async () => {
    const { priceOracle } = await loadFixture(fixture);
    await expect(priceOracle.read.premium(["foobar", 0n, 0n])).resolves.toEqual(0n);
    await expect(priceOracle.read.price(["foobar", 0n, 0n])).resolves.toHaveProperty("base", 0n);
  });
  it("should not specify a premium for renewals", async () => {
    ...
    await expect(priceOracle.read.premium(["foobar", timestamp, 0n])).resolves.toEqual(0n);
    await expect(priceOracle.read.price(["foobar", timestamp, 0n])).resolves.toHaveProperty("base", 0n);
  });
  it("should specify the maximum premium at the moment of expiration", async () => {
    const { priceOracle, publicClient } = await loadFixture(fixture);
    const timestamp = await publicClient.getBlock().then(b => b.timestamp - 90n * BigInt(DAY));
    const expectedPrice = (START_PRICE_WITH_FACTOR - LAST_VALUE_WITH_FACTOR) / 2n; // ETH at $2 for $1 mil in 18 decimal precision
    await expect(priceOracle.read.premium(["foobar", timestamp, 0n])).resolves.toEqual(expectedPrice);
    await expect(priceOracle.read.price(["foobar", timestamp, 0n])).resolves.toHaveProperty("premium", expectedPrice);
  });
  it("should specify the correct price after 2.5 days and 1 year registration", async () => {
    const { priceOracle, publicClient } = await loadFixture(fixture);
    const timestamp = await publicClient.getBlock().then(b => b.timestamp - (90n * DAY + 2n * DAY + DAY / 2n));
    const lengthOfRegistration = DAY * 365n;
    const expectedPremium = (exponentialReduceFloatingPoint(START_PRICE, 2.5) / 2).toPrecision(15);
    await expect(
      priceOracle.read
  ...
  it("should produce a 0 premium at the end of the decay period", async () => {
    const { priceOracle, publicClient } = await loadFixture(fixture);
    const timestamp = await publicClient.getBlock().then(b => b.timestamp - 90n * DAY);
    await expect(priceOracle.read.premium(["foobar", timestamp - LAST_DAY * DAY + 1n, 0n])).resolves.toBeGreaterThan(
      0n,
    );
    await expect(priceOracle.read.premium(["foobar", timestamp - LAST_DAY * DAY, 0n])).resolves.toEqual(0n);
  });
  ...
  it("should not be beyond a certain amount of inaccuracy from floating point calc", async () => {
    ...
    const result = await Promise.all(
      Array.from({ length: Number(DAY * LAST_DAY) / interval }).map(async (_, i) => {
        const seconds = i * interval;
        const time = timestamp - BigInt(seconds);
        const contractResult = await priceOracle.read.premium(["foobar", time, 0n]).then(p => Number(p) / 1e18);
        const jsResult = exponentialReduceFloatingPoint(START_PRICE, seconds / 86400) / 2;
      ...
      results.reduce(
        (prev, curr) => {
          ...
          if (curr.percent > prev.percentMax && curr.absoluteDifference > 0.01) {
