...
async function fixture() {
  ...
  await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("reverse"), accounts.ownerAccount.address]);
  ...
  await ensRegistry.write.setSubnodeOwner([zeroHash, labelhash("eth"), baseRegistrar.address]);
  const dummyOracle = await hre.viem.deployContract("DummyOracle", [100000000n]);
  const priceOracle = await hre.viem.deployContract("StablePriceOracle", [dummyOracle.address, [0n, 0n, 4n, 2n, 1n]]);
...
describe("ETHRegistrarController", () => {
  ...
  it("should permit new registrations", async () => {
    ...
    await expect(publicClient.getBalance({ address: ethRegistrarController.address })).resolves.toEqual(
      REGISTRATION_TIME + balanceBefore,
    );
  });
  ...
  it("should permit a registration with resolver but no records", async () => {
    const { ensRegistry, ethRegistrarController, publicResolver, publicClient, registrantAccount } = await loadFixture(
      fixture,
    );
    const { args, params } = await commitName(
      { ethRegistrarController },
  ...
  it("should allow anyone to renew a name and change fuse expiry", async () => {
    const { baseRegistrar, ethRegistrarController, nameWrapper, publicClient, registrantAccount } = await loadFixture(
      fixture,
    );
    await registerName(
      { ethRegistrarController },
    ...
    const fuseExpiry = await nameWrapper.read.getData([hexToBigInt(nodehash)]).then(d => d[2]);
    const expires = await baseRegistrar.read.nameExpires([labelId("newname")]);
    const balanceBefore = await publicClient.getBalance({
      address: ethRegistrarController.address,
    });
    const duration = 86400n;
    const { base: price } = await ethRegistrarController.read.rentPrice(["newname", duration]);
    await ethRegistrarController.write.renew(["newname", duration], {
      value: price,
    });
    const newExpires = await baseRegistrar.read.nameExpires([labelId("newname")]);
    const newFuseExpiry = await nameWrapper.read.getData([hexToBigInt(nodehash)]).then(d => d[2]);
  ...
  it("non wrapped names can renew", async () => {
    ...
    await baseRegistrar.write.register([tokenId, ownerAccount.address, duration]);
    ...
    const { base: price } = await ethRegistrarController.read.rentPrice([label, duration]);
  ...
  it("should set the reverse record of the account", async () => {
    ...
    await expect(publicResolver.read.name([namehash(getReverseNode(ownerAccount.address))])).resolves.toEqual(
      "reverse.eth",
    );
  });
  it("should not set the reverse record of the account when set to false", async () => {
    ...
    await expect(publicResolver.read.name([namehash(getReverseNode(registrantAccount.address))])).resolves.toEqual("");
  });
  it("should auto wrap the name and set the ERC721 owner to the wrapper", async () => {
    const { ensRegistry, baseRegistrar, ethRegistrarController, nameWrapper, registrantAccount } = await loadFixture(
      fixture,
    );
    const label = "wrapper";
    ...
    await expect(nameWrapper.read.ownerOf([hexToBigInt(namehash(name))])).resolves.toEqualAddress(
  ...
  it("should auto wrap the name and allow fuses and expiry to be set", async () => {
    ...
    const [, fuses, expiry] = await nameWrapper.read.getData([hexToBigInt(namehash(name))]);
  ...
  it("approval should reduce gas for registration", async () => {
    ...
    await publicResolver.write.setApprovalForAll([ethRegistrarController.address, true], {
      account: registrantAccount,
    });
  ...
  it("should not permit new registrations with non resolver function calls", async () => {
    ...
    const callData = [
      encodeFunctionData({
        ...
        args: [hexToBigInt(node), registrantAccount.address, secondTokenDuration],
      }),
    ];
    const { args } = await commitName(
      { ethRegistrarController },
