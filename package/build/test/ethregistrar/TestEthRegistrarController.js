...
import { encodeFunctionData, hexToBigInt, labelhash, namehash, zeroAddress, zeroHash, } from 'viem';
import { DAY, FUSES } from '../fixtures/constants.js';
import { getReverseNode } from '../fixtures/getReverseNode.js';
import { commitName, getDefaultRegistrationOptions, getRegisterNameParameterArray, registerName, } from '../fixtures/registerName.js';
const REGISTRATION_TIME = 28n * DAY;
const BUFFERED_REGISTRATION_COST = REGISTRATION_TIME + 3n * DAY;
const GRACE_PERIOD = 90n * DAY;
const getAccounts = async () => {
    const [ownerClient, registrantClient, otherClient] = await hre.viem.getWalletClients();
    return {
...
async function fixture() {
    ...
    const baseRegistrar = await hre.viem.deployContract('BaseRegistrarImplementation', [ensRegistry.address, namehash('eth')]);
...
describe('ETHRegistrarController', () => {
    it('should report label validity', async () => {
        ...
        for (const label in checkLabels) {
            await expect(ethRegistrarController.read.valid([label])).resolves.toEqual(checkLabels[label]);
        }
    });
    it('should report unused names as available', async () => {
        const { ethRegistrarController } = await loadFixture(fixture);
        await expect(ethRegistrarController.read.available(['available'])).resolves.toEqual(true);
    });
    it('should permit new registrations', async () => {
        const { ethRegistrarController, publicClient, registrantAccount } = await loadFixture(fixture);
        const balanceBefore = await publicClient.getBalance({
            address: ethRegistrarController.address,
        });
        const { args, params } = await commitName({ ethRegistrarController }, {
            label: 'newname',
        ...
        await expect(ethRegistrarController)
            ...
            .withArgs(params.label, labelhash(params.label), params.ownerAddress, params.duration, 0n, timestamp + params.duration);
        await expect(publicClient.getBalance({ address: ethRegistrarController.address })).resolves.toEqual(REGISTRATION_TIME + balanceBefore);
    });
    it('should revert when not enough ether is transferred', async () => {
        const { ethRegistrarController, registrantAccount } = await loadFixture(fixture);
        const { args } = await commitName({ ethRegistrarController }, {
            label: 'newname',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        });
    ...
    it('should report registered names as unavailable', async () => {
        ...
        await expect(ethRegistrarController.read.available(['newname'])).resolves.toEqual(false);
    });
    it('should permit new registrations with resolver and records', async () => {
        const { ensRegistry, baseRegistrar, nameWrapper, ethRegistrarController, callData, publicResolver, publicClient, registrantAccount, } = await loadFixture(fixture);
        const { args, params } = await commitName({ ethRegistrarController }, {
            label: 'newconfigname',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        ...
        await expect(ethRegistrarController)
            ...
            .toEmitEvent('NameRegistered')
            .withArgs(params.label, labelhash(params.label), params.ownerAddress, params.duration, 0n, timestamp + params.duration);
        await expect(publicClient.getBalance({ address: ethRegistrarController.address })).resolves.toEqual(REGISTRATION_TIME);
        const nodehash = namehash('newconfigname.eth');
        await expect(ensRegistry.read.resolver([nodehash])).resolves.toEqualAddress(publicResolver.address);
        await expect(ensRegistry.read.owner([nodehash])).resolves.toEqualAddress(nameWrapper.address);
        await expect(baseRegistrar.read.ownerOf([labelId('newconfigname')])).resolves.toEqualAddress(nameWrapper.address);
        await expect(publicResolver.read.addr([nodehash])).resolves.toEqualAddress(registrantAccount.address);
        await expect(publicResolver.read.text([nodehash, 'url'])).resolves.toEqual('ethereum.com');
        await expect(nameWrapper.read.ownerOf([hexToBigInt(nodehash)])).resolves.toEqualAddress(registrantAccount.address);
    });
    it('should not permit new registrations with data and 0 resolver', async () => {
        const { ethRegistrarController, registrantAccount, callData } = await loadFixture(fixture);
        await expect(ethRegistrarController)
            .read('makeCommitment', getRegisterNameParameterArray(await getDefaultRegistrationOptions({
            label: 'newconfigname',
            ownerAddress: registrantAccount.address,
            data: callData,
        })))
    ...
    it('should not permit new registrations with records updating a different name', async () => {
        const { ethRegistrarController, publicResolver, registrantAccount } = await loadFixture(fixture);
        const { args } = await commitName({ ethRegistrarController }, {
            label: 'awesome',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        ...
        await expect(ethRegistrarController)
            .write('register', args, { value: BUFFERED_REGISTRATION_COST })
            .toBeRevertedWithString('multicall: All records must have a matching namehash');
    });
    ...
    it('should permit a registration with resolver but no records', async () => {
        const { ensRegistry, ethRegistrarController, publicResolver, publicClient, registrantAccount, } = await loadFixture(fixture);
        const { args, params } = await commitName({ ethRegistrarController }, {
            label: 'newconfigname',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        ...
        const nodehash = namehash('newconfigname.eth');
        await expect(ensRegistry.read.resolver([nodehash])).resolves.toEqualAddress(publicResolver.address);
        await expect(publicResolver.read.addr([nodehash])).resolves.toEqual(zeroAddress);
        await expect(publicClient.getBalance({ address: ethRegistrarController.address })).resolves.toEqual(REGISTRATION_TIME);
    });
    it('should include the owner in the commitment', async () => {
        const { ethRegistrarController, registrantAccount, otherAccount } = await loadFixture(fixture);
        let { args, params } = await commitName({ ethRegistrarController }, {
            label: 'newname',
            duration: REGISTRATION_TIME,
    ...
    it('should reject duplicate registrations', async () => {
        const { ethRegistrarController, registrantAccount } = await loadFixture(fixture);
        const label = 'newname';
        await registerName({ ethRegistrarController }, {
            label,
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        });
    ...
    it('should reject for expired commitments', async () => {
        ...
        const { args, hash } = await commitName({ ethRegistrarController }, {
            label: 'newname',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        });
        const minCommitmentAge = await ethRegistrarController.read.minCommitmentAge();
        const maxCommitmentAge = await ethRegistrarController.read.maxCommitmentAge();
    ...
    it('should allow anyone to renew a name and change fuse expiry', async () => {
        const { baseRegistrar, ethRegistrarController, nameWrapper, publicClient, registrantAccount, } = await loadFixture(fixture);
        await registerName({ ethRegistrarController }, {
            label: 'newname',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
    ...
    it('non wrapped names can renew', async () => {
        const { nameWrapper, baseRegistrar, ethRegistrarController, ownerAccount } = await loadFixture(fixture);
        const label = 'newname';
        ...
        await expect(nameWrapper.read.ownerOf([hexToBigInt(nodehash)])).resolves.toEqual(zeroAddress);
        await expect(baseRegistrar.read.ownerOf([tokenId])).resolves.toEqualAddress(ownerAccount.address);
    ...
    it('should allow anyone to withdraw funds and transfer to the registrar owner', async () => {
        const { ethRegistrarController, ownerAccount, publicClient } = await loadFixture(fixture);
        await registerName({ ethRegistrarController }, {
            label: 'newname',
            duration: REGISTRATION_TIME,
        ...
        await expect(publicClient.getBalance({ address: ethRegistrarController.address })).resolves.toEqual(0n);
    });
    it('should set the reverse record of the account', async () => {
        const { ethRegistrarController, publicResolver, registrantAccount, ownerAccount, } = await loadFixture(fixture);
        await registerName({ ethRegistrarController }, {
            label: 'reverse',
            duration: REGISTRATION_TIME,
            ownerAddress: registrantAccount.address,
        ...
        await expect(publicResolver.read.name([
    ...
    it('should auto wrap the name and set the ERC721 owner to the wrapper', async () => {
        const { ensRegistry, baseRegistrar, ethRegistrarController, nameWrapper, registrantAccount, } = await loadFixture(fixture);
        const label = 'wrapper';
        ...
        await expect(nameWrapper.read.ownerOf([hexToBigInt(namehash(name))])).resolves.toEqualAddress(registrantAccount.address);
        await expect(ensRegistry.read.owner([namehash(name)])).resolves.toEqualAddress(nameWrapper.address);
        await expect(baseRegistrar.read.ownerOf([labelId(label)])).resolves.toEqualAddress(nameWrapper.address);
    });
    it('should auto wrap the name and allow fuses and expiry to be set', async () => {
        const { publicClient, ethRegistrarController, nameWrapper, registrantAccount, } = await loadFixture(fixture);
        const label = 'fuses';
        const name = label + '.eth';
        await registerName({ ethRegistrarController }, {
            label,
        ...
        expect(fuses).toEqual(FUSES.PARENT_CANNOT_CONTROL | FUSES.CANNOT_UNWRAP | FUSES.IS_DOT_ETH);
        expect(expiry).toEqual(REGISTRATION_TIME + GRACE_PERIOD + block.timestamp);
    });
    it('approval should reduce gas for registration', async () => {
        const { publicClient, ensRegistry, baseRegistrar, ethRegistrarController, nameWrapper, registrantAccount, publicResolver, } = await loadFixture(fixture);
        const label = 'other';
        const name = label + '.eth';
        ...
        await publicResolver.write.setApprovalForAll([ethRegistrarController.address, true], { account: registrantAccount });
        ...
        await expect(nameWrapper.read.ownerOf([hexToBigInt(node)])).resolves.toEqualAddress(registrantAccount.address);
        await expect(ensRegistry.read.owner([node])).resolves.toEqualAddress(nameWrapper.address);
        await expect(baseRegistrar.read.ownerOf([labelId(label)])).resolves.toEqualAddress(nameWrapper.address);
        await expect(publicResolver.read.addr([node])).resolves.toEqualAddress(registrantAccount.address);
    });
    it('should not permit new registrations with non resolver function calls', async () => {
        const { baseRegistrar, ethRegistrarController, registrantAccount, publicResolver, } = await loadFixture(fixture);
