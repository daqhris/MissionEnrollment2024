...
async function fixture() {
  ...
  const bulkRenewal = await hre.viem.deployContract("BulkRenewal", [ensRegistry.address]);
  ...
  await publicResolver.write.setInterface([namehash("eth"), interfaceId, controller.address]);
  await ensRegistry.write.setOwner([namehash("eth"), baseRegistrar.address]);
  // Register some names
  for (const name of ["test1", "test2", "test3"]) {
    await baseRegistrar.write.register([toLabelId(name), accounts[1].address, 31536000n]);
  }
...
describe("BulkRenewal", () => {
  it("should return the cost of a bulk renewal", async () => {
    ...
    await expect(bulkRenewal.read.rentPrice([["test1", "test2"], 86400n])).resolves.toEqual(86400n * 2n);
  });
  ...
  it("should permit bulk renewal of names", async () => {
    ...
    await expect(publicClient.getBalance({ address: bulkRenewal.address })).resolves.toEqual(0n);
  });
});
