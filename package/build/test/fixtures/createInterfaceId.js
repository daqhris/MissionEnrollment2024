...
import { bytesToHex, hexToBytes, toFunctionHash } from "viem";
...
const getSolidityReferenceInterfaceAbi = async interfaceName => {
  ...
  if (!fullyQualifiedInterfaceName) throw new Error("Couldn't find fully qualified interface name");
...
export const createInterfaceId = iface => {
  const bytesId = iface
...
export const getInterfaceId = async interfaceName => {
