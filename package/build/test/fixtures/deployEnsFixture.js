...
export async function deployEnsStack() {
    ...
    const baseRegistrarImplementation = await hre.viem.deployContract('BaseRegistrarImplementation', [ensRegistry.address, namehash('eth')]);
    ...
    const staticMetadataService = await hre.viem.deployContract('StaticMetadataService', ['http://localhost:8080/name/0x{id}']);
