import { bytesToHex, labelhash as labelhashBytes32, stringToBytes } from "viem";
...
export function packetToBytes(packet) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    if (encoded.byteLength > 255) encoded = stringToBytes(encodeLabelhash(labelhashBytes32(list[i])));
...
export const dnsEncodeName = name => bytesToHex(packetToBytes(name));
