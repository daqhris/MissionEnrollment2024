...
export const testEntries = [...realEntries, dummyEntry];
export const encodedAnchors = `0x${testEntries.map(entry => packet.answer.encode(entry).toString("hex")).join("")}`;
export const encodedRealAnchors = `0x${realEntries.map(entry => packet.answer.encode(entry).toString("hex")).join("")}`;
