...
describe('NameEncoder', () => {
    describe('encodeName()', () => {
        it('should encode a name', async () => {
            ...
            await expect(nameEncoder.read.encodeName(['foo.eth'])).resolves.toMatchObject([dnsEncodeName('foo.eth'), namehash('foo.eth')]);
        });
        it('should encode an empty name', async () => {
            const { nameEncoder } = await loadFixture(fixture);
            await expect(nameEncoder.read.encodeName([''])).resolves.toMatchObject([
                `${dnsEncodeName('')}00` /* uhhh idk if its meant to be like this but leaving it for now */,
                namehash(''),
            ]);
        });
        it('should encode a long name', async () => {
            const { nameEncoder } = await loadFixture(fixture);
            await expect(nameEncoder.read.encodeName(['something.else.test.eth'])).resolves.toMatchObject([
