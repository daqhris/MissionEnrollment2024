...
import { decodeFunctionResult, encodeAbiParameters, encodeErrorResult, encodeFunctionData, encodeFunctionResult, getAddress, getContract, labelhash, namehash, parseAbiItem, parseAbiParameters, toFunctionSelector, zeroAddress, zeroHash, } from 'viem';
...
const encodeExtraData = ({ isWildcard, resolver, gateways, metadata, extraDatas, }) => encodeAbiParameters([
...
async function fixture() {
    ...
    await ensRegistry.write.setResolver([getReverseNodeHash(accounts[10].address), oldResolver.address], { account: accounts[10] });
...
describe('UniversalResolver', () => {
    describe('findResolver()', () => {
        ...
        it('should allow encoded labels', async () => {
            const { universalResolver, publicResolver } = await loadFixture(fixture);
            await expect(universalResolver.read.findResolver([
    ...
    describe('resolve()', () => {
        ...
        it('should return a wrapped revert if the resolver reverts with OffchainLookup', async () => {
            ...
            await expect(universalResolver)
                ...
                .withArgs(getAddress(universalResolver.address), ['http://universal-offchain-resolver.local'], queryCalldata, toFunctionSelector('function resolveSingleCallback(bytes,bytes)'), extraData);
        });
        ...
        it('should return a wrapped revert with resolve() wrapped calls in extraData when combining onchain and offchain', async () => {
            ...
            await expect(universalResolver)
                ...
                .toBeRevertedWithCustomError('OffchainLookup')
                .withArgs(getAddress(universalResolver.address), ['http://universal-offchain-resolver.local'], queryCalldata, toFunctionSelector('function resolveCallback(bytes,bytes)'), extraData);
        });
        it('should revert OffchainLookup via UniversalResolver + OffchainDNSResolver', async () => {
            const { universalResolver, batchGatewayAbi, publicResolver, ensRegistry, root, accounts, } = await loadFixture(fixture);
            ...
            const offchainDnsResolver = await hre.viem.deployContract('OffchainDNSResolver', [ensRegistry.address, dnssec.address, OFFCHAIN_DNS_GATEWAY]);
            ...
            const innerExtraData = encodeAbiParameters(parseAbiParameters('bytes, bytes, bytes4'), [dnsEncodeName(name), addrCall, '0x00000000']);
            const extraData = encodeExtraData({
                isWildcard: true,
    ...
    describe('resolveSingleCallback()', () => {
        it('should resolve a record via a callback from offchain lookup', async () => {
            ...
            const [encodedAddr, resolverAddress] = await universalResolver.read.resolveSingleCallback([
                response,
                extraData,
            ]);
    ...
    describe('resolveCallback', () => {
        it('should resolve records via a callback from offchain lookup', async () => {
            ...
            const [[encodedAddr, encodedAddrTwo], resolverAddress] = await universalResolver.read.resolveCallback([response, extraData]);
            expect(resolverAddress).toEqualAddress(offchainResolver.address);
        ...
        it('should allow response at non-0 extraData index', async () => {
            ...
            const [[encodedResult, encodedResultTwo], resolverAddress] = await universalResolver.read.resolveCallback([response, extraData]);
            expect(resolverAddress).toEqualAddress(offchainResolver.address);
    ...
    describe('reverseCallback', () => {
        it('should revert with metadata for initial forward resolution if required', async () => {
            const { universalResolver, offchainResolver, batchGatewayAbi } = await loadFixture(fixture);
            const metadata = encodeAbiParameters([{ type: 'string' }, { type: 'address' }], ['offchain.test.eth', offchainResolver.address]);
            const addrCall = encodeFunctionData({
            ...
            await expect(universalResolver)
                ...
                .toBeRevertedWithCustomError('OffchainLookup')
                .withArgs(getAddress(universalResolver.address), ['http://universal-offchain-resolver.local'], expect.anyValue, toFunctionSelector('function reverseCallback(bytes,bytes)'), extraDataForResult);
        });
