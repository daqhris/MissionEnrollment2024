...
import { getReverseNode, getReverseNodeHash } from "../fixtures/getReverseNode.js";
...
const encodeExtraData = ({ isWildcard, resolver, gateways, metadata, extraDatas }) =>
  encodeAbiParameters(
    [
...
async function fixture() {
  const accounts = await hre.viem.getWalletClients().then(clients => clients.map(c => c.account));
  ...
  const offchainResolver = await hre.viem.deployContract("DummyOffchainResolver", []);
  const oldResolver = await hre.viem.deployContract("DummyOldResolver", []);
  const revertResolver = await hre.viem.deployContract("DummyRevertResolver", []);
  const legacyResolver = await hre.viem.deployContract("LegacyResolver", []);
  const createTestEthSub = async ({ label, resolverAddress = zeroAddress }) => {
  ...
  for (let i = 0; i < 5; i += 1) {
    const parent = name;
    const label = `sub${i}`;
    await ensRegistry.write.setSubnodeOwner([namehash(parent), labelhash(label), accounts[0].address]);
    name = `${label}.${name}`;
  }
  await publicResolver.write.setAddr([namehash("test.eth"), accounts[1].address]);
  await publicResolver.write.setText([namehash("test.eth"), "foo", "bar"]);
  await reverseRegistrar.write.claim([accounts[0].address]);
  await ensRegistry.write.setResolver([getReverseNodeHash(accounts[0].address), publicResolver.address]);
  await publicResolver.write.setName([getReverseNodeHash(accounts[0].address), "test.eth"]);
  await reverseRegistrar.write.claim([accounts[10].address], {
    account: accounts[10],
  });
  await ensRegistry.write.setResolver([getReverseNodeHash(accounts[10].address), oldResolver.address], {
    account: accounts[10],
  });
  const batchGatewayAbi = await hre.artifacts.readArtifact("BatchGateway").then(({ abi }) => abi);
  return {
    ensRegistry,
    nameWrapper,
    reverseRegistrar,
...
describe("UniversalResolver", () => {
  describe("findResolver()", () => {
    it("should find an exact match resolver", async () => {
      ...
      await expect(universalResolver.read.findResolver([dnsEncodeName("test.eth")])).resolves.toMatchObject([
    ...
    it("should find a resolver on a parent name", async () => {
      const { universalResolver, publicResolver } = await loadFixture(fixture);
      await expect(universalResolver.read.findResolver([dnsEncodeName("foo.test.eth")])).resolves.toMatchObject([
        getAddress(publicResolver.address),
    ...
    it("should choose the resolver closest to the leaf", async () => {
      ...
      await expect(universalResolver.read.findResolver([dnsEncodeName("sub.test.eth")])).resolves.toMatchObject([
    ...
    it("should allow encoded labels", async () => {
      ...
      await expect(
        universalResolver.read.findResolver([
          dnsEncodeName("[9c22ff5f21f0b81b113e63f7db6da94fedef11b2119b4088b89664fb9a3cb658].eth"),
        ]),
      ).resolves.toMatchObject([getAddress(publicResolver.address), namehash("test.eth"), 0n]);
    });
    it("should find a resolver many levels up", async () => {
      const { universalResolver, publicResolver } = await loadFixture(fixture);
      await expect(
        universalResolver.read.findResolver([dnsEncodeName("sub4.sub3.sub2.sub1.sub0.test.eth")]),
  ...
  describe("resolve()", () => {
    it("should resolve a record via legacy methods", async () => {
      const { universalResolver, publicResolver, accounts } = await loadFixture(fixture);
      const args = [namehash("test.eth")];
      const data = encodeFunctionData({
        abi: publicResolver.abi,
        functionName: "addr",
        args,
      });
      const [result] = await universalResolver.read.resolve([dnsEncodeName("test.eth"), data]);
    ...
    it("should throw if a resolver is not a contract", async () => {
      ...
      await expect(universalResolver)
        .read("resolve", [dnsEncodeName("non-contract-resolver.test.eth"), data])
    ...
    it("should resolve a record if supportsInterface() throws", async () => {
      const { universalResolver, publicResolver, legacyResolver } = await loadFixture(fixture);
      const args = [namehash("legacy-resolver.eth")];
      const data = encodeFunctionData({
        abi: publicResolver.abi,
        functionName: "addr",
        args,
    ...
    it("should not run out of gas if calling a non-existent function on a legacy resolver", async () => {
      const { universalResolver, publicResolver, legacyResolver } = await loadFixture(fixture);
      const args = [namehash("legacy-resolver.eth"), 60n];
      const data = encodeFunctionData({
        abi: publicResolver.abi,
        functionName: "addr",
        args,
    ...
    it("should return a wrapped revert if the resolver reverts with OffchainLookup", async () => {
      const { universalResolver, publicResolver, offchainResolver, batchGatewayAbi } = await loadFixture(fixture);
      ...
      const extraData = encodeExtraData({
        ...
        extraDatas: [
          {
            callbackFunction: toFunctionSelector("function resolveCallback(bytes,bytes)"),
            data: callData,
          },
        ],
      });
    ...
    it("should use custom gateways when specified", async () => {
      const { universalResolver, publicResolver } = await loadFixture(fixture);
      const args = [namehash("offchain.test.eth"), 60n];
      const data = encodeFunctionData({
        abi: publicResolver.abi,
        functionName: "addr",
        args,
      ...
      await expect(universalResolver)
        .read("resolve", [dnsEncodeName("offchain.test.eth"), data, ["https://custom.local"]])
        .toBeRevertedWithCustomError("OffchainLookup")
        .withArgs(expect.anyValue, ["https://custom.local"], expect.anyValue, expect.anyValue, expect.anyValue);
    });
    it("should return a wrapped revert with resolve() wrapped calls in extraData when combining onchain and offchain", async () => {
      ...
      await expect(universalResolver)
        .read("resolve", [dnsEncodeName("offchain.test.eth"), [addrCall, onchainDataCall]])
        .toBeRevertedWithCustomError("OffchainLookup")
        .withArgs(
          getAddress(universalResolver.address),
          ["http://universal-offchain-resolver.local"],
    ...
    it("should revert OffchainLookup via UniversalResolver + OffchainDNSResolver", async () => {
      const { universalResolver, batchGatewayAbi, publicResolver, ensRegistry, root, accounts } = await loadFixture(
        fixture,
      );
      const OFFCHAIN_DNS_GATEWAY = "https://localhost:8000/lookup";
      const dnssec = await hre.viem.deployContract("DNSSECImpl", [encodedRealAnchors]);
      const suffixes = await hre.viem.deployContract("SimplePublicSuffixList", []);
      const dnsGatewayAbi = await hre.artifacts.readArtifact("IDNSGateway").then(({ abi }) => abi);
      await suffixes.write.addPublicSuffixes([[dnsEncodeName("test")]]);
      const offchainDnsResolver = await hre.viem.deployContract("OffchainDNSResolver", [
        ensRegistry.address,
        dnssec.address,
      ...
      const innerExtraData = encodeAbiParameters(parseAbiParameters("bytes, bytes, bytes4"), [
        dnsEncodeName(name),
        addrCall,
        "0x00000000",
      ]);
  ...
  describe("batch", () => {
    it("should resolve multiple records onchain", async () => {
      const { universalResolver, publicResolver, accounts } = await loadFixture(fixture);
      const addrArgs = [namehash("test.eth")];
      ...
      const [[textResultEncoded, addrResultEncoded]] = await universalResolver.read.resolve([
    ...
    it("should resolve multiple records offchain", async () => {
      const { universalResolver, publicResolver, offchainResolver, batchGatewayAbi } = await loadFixture(fixture);
      const addrArgs = [namehash("offchain.test.eth")];
      const textData = encodeFunctionData({
        abi: publicResolver.abi,
        functionName: "text",
      ...
      await expect(universalResolver)
        .read("resolve", [dnsEncodeName("offchain.test.eth"), [textData, addrData]])
        .toBeRevertedWithCustomError("OffchainLookup")
        .withArgs(
          getAddress(universalResolver.address),
          ["http://universal-offchain-resolver.local"],
  ...
  describe("resolveSingleCallback()", () => {
    it("should resolve a record via a callback from offchain lookup", async () => {
      ...
      const [encodedAddr, resolverAddress] = await universalResolver.read.resolveSingleCallback([response, extraData]);
    ...
    it("should propagate HttpError", async () => {
      const { universalResolver, offchainResolver, batchGatewayAbi } = await loadFixture(fixture);
      const publicClient = await hre.viem.getPublicClient();
      const universalResolverWithHttpError = getContract({
        abi: [...universalResolver.abi, parseAbiItem("error HttpError((uint16,string)[])")],
  ...
  describe("resolveCallback", () => {
    it("should resolve records via a callback from offchain lookup", async () => {
      ...
      const [[encodedAddr, encodedAddrTwo], resolverAddress] = await universalResolver.read.resolveCallback([
        response,
        extraData,
      ]);
      expect(resolverAddress).toEqualAddress(offchainResolver.address);
    ...
    it("should not revert if there is an error in a call", async () => {
      ...
      const [[encodedResult], resolverAddress] = await universalResolver.read.resolveCallback([response, extraData]);
      expect(resolverAddress).toEqualAddress(offchainResolver.address);
    ...
    it("should allow response at non-0 extraData index", async () => {
      const { universalResolver, offchainResolver, publicResolver, batchGatewayAbi } = await loadFixture(fixture);
      ...
      const [[encodedResult, encodedResultTwo], resolverAddress] = await universalResolver.read.resolveCallback([
        response,
        extraData,
      ]);
      expect(resolverAddress).toEqualAddress(offchainResolver.address);
    ...
    it("should handle a non-existent function on an offchain resolver", async () => {
      ...
      const [[addr, text], resolverAddress] = await universalResolver.read.resolveCallback([response, extraData]);
  ...
  describe("reverseCallback", () => {
    ...
    it("should resolve address record via a callback from offchain lookup", async () => {
      ...
      const [name, a1, a2, a3] = await universalResolver.read.reverseCallback([response, extraData]);
  ...
  describe("reverse()", () => {
    it("should resolve a reverse record with name and resolver address", async () => {
      const { universalResolver, accounts, publicResolver } = await loadFixture(fixture);
      const [name, resolvedAddress, reverseResolverAddress, resolverAddress] = await universalResolver.read.reverse([
