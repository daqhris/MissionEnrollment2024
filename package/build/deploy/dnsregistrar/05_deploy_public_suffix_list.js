...
const func = async function (hre) {
  ...
  if (owner !== undefined && owner.address !== deployer.address && listOwner !== owner.address) {
  ...
  for (let i = 0; i < suffixes.length; i += 100) {
    const batch = suffixes.slice(i, i + 100).map(suffix => dnsEncodeName(suffix));
  ...
  console.log(`Waiting on ${transactionHashes.length} suffix-setting transactions to complete...`);
  await Promise.all(transactionHashes.map(hash => viem.waitForTransactionSuccess(hash)));
};
