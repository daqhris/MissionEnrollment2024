...
const func = async function (hre) {
  ...
  for (const [id, alg] of Object.entries(algorithms)) {
    const deployedAlgorithmAddress = await deployments.get(alg).then(d => d.address);
    ...
    if (deployedAlgorithmAddress != currentAlgorithmAddress) {
      const hash = await dnssec.write.setAlgorithm([parseInt(id), deployedAlgorithmAddress]);
      transactions.push(hash);
    }
  }
  for (const [id, digest] of Object.entries(digests)) {
    const deployedDigestAddress = await deployments.get(digest).then(d => d.address);
    const currentDigestAddress = await dnssec.read.digests([parseInt(id)]);
    if (deployedDigestAddress != currentDigestAddress) {
      const hash = await dnssec.write.setDigest([parseInt(id), deployedDigestAddress]);
      transactions.push(hash);
    }
  }
  console.log(`Waiting on ${transactions.length} transactions setting DNSSEC parameters`);
  await Promise.all(transactions.map(async hash => viem.waitForTransactionSuccess(hash)));
};
