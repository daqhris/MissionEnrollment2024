...
const func = async function (hre) {
  ...
  const reverseRegistrarSetDefaultResolverHash = await reverseRegistrar.write.setDefaultResolver([
  ...
  if (resolverEthOwner === owner.address) {
    ...
    const setResolverHash = await registry.write.setResolver([namehash("resolver.eth"), publicResolver.address]);
    console.log(`Setting resolver for resolver.eth to PublicResolver (tx: ${setResolverHash})...`);
    await viem.waitForTransactionSuccess(setResolverHash);
    const setAddrHash = await publicResolver.write.setAddr([namehash("resolver.eth"), publicResolver.address]);
    console.log(`Setting address for resolver.eth to PublicResolver (tx: ${setAddrHash})...`);
