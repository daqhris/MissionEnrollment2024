...
const func = async function (hre) {
  ...
  console.log(`Setting owner of root node to root contract (tx: ${setOwnerHash})...`);
  await viem.waitForTransactionSuccess(setOwnerHash);
  const rootOwner = await root.read.owner();
  switch (rootOwner) {
    case deployer.address:
      const transferOwnershipHash = await root.write.transferOwnership([owner.address]);
      console.log(`Transferring root ownership to final owner (tx: ${transferOwnershipHash})...`);
    ...
    case owner.address:
      ...
      if (!ownerIsRootController) {
        const setControllerHash = await root.write.setController([owner.address, true], { account: owner.account });
        console.log(`Setting final owner as controller on root contract (tx: ${setControllerHash})...`);
