...
const func = async function (hre) {
    ...
    if (owner.address !== deployer.address) {
        ...
        console.log(`Transferring ownership of ETHRegistrarController to ${owner.address} (tx: ${hash})...`);
    ...
    console.log(`Adding ETHRegistrarController as a controller of NameWrapper (tx: ${nameWrapperSetControllerHash})...`);
    await viem.waitForTransactionSuccess(nameWrapperSetControllerHash);
    const reverseRegistrarSetControllerHash = await reverseRegistrar.write.setController([controller.address, true]);
    console.log(`Adding ETHRegistrarController as a controller of ReverseRegistrar (tx: ${reverseRegistrarSetControllerHash})...`);
    ...
    console.log(`Setting ETHRegistrarController interface ID ${interfaceId} on .eth resolver (tx: ${setInterfaceHash})...`);
