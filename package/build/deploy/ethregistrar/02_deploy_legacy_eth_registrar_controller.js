const func = async function (hre) {
    ...
    const controller = await viem.deploy('LegacyETHRegistrarController', [registrar.address, priceOracle.address, 60n, 86400n], {
        artifact: await deployments.getArtifact('ETHRegistrarController_mainnet_9380471'),
    });
    const registrarAddControllerHash = await registrar.write.addController([controller.address], { account: owner.account });
    console.log(`Adding controller as controller on registrar (tx: ${registrarAddControllerHash})...`);
    await viem.waitForTransactionSuccess(registrarAddControllerHash);
    const reverseRegistrarSetControllerHash = await reverseRegistrar.write.setController([controller.address, true], { account: owner.account });
    console.log(`Setting controller of ReverseRegistrar to controller (tx: ${reverseRegistrarSetControllerHash})...`);
