...
const func = async function (hre) {
  ...
  if (owner.address !== deployer.address) {
    ...
    console.log(`Transferring ownership of NameWrapper to ${owner.address} (tx: ${hash})...`);
    await viem.waitForTransactionSuccess(hash);
  }
  // Only attempt to make controller etc changes directly on testnets
  if (network.name === "mainnet") return;
  const addControllerHash = await registrar.write.addController([nameWrapper.address]);
  console.log(`Adding NameWrapper as controller on registrar (tx: ${addControllerHash})...`);
  ...
  console.log(`Setting NameWrapper interface ID ${interfaceId} on .eth resolver (tx: ${setInterfaceHash})...`);
  await viem.waitForTransactionSuccess(setInterfaceHash);
};
func.id = "name-wrapper";
func.tags = ["wrapper", "NameWrapper"];
func.dependencies = ["StaticMetadataService", "registry", "ReverseRegistrar", "OwnedResolver"];
export default func;
