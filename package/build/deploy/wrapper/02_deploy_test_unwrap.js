...
const func = async function (hre) {
  ...
  const testnetWrapperAddresses = TESTNET_WRAPPER_ADDRESSES[network.name];
  ...
  for (const wrapperAddress of testnetWrapperAddresses) {
    ...
    const isApprovedWrapper = await testUnwrap.read.approvedWrapper([wrapperAddress]);
    ...
    if (!isUpgradeSet) {
      ...
      if (!canModifyWrapper && !canModifyTestUnwrap) {
        ...
        const hash = await wrapper.write.setUpgradeContract([testUnwrap.address]);
        console.log(`Setting upgrade contract for ${wrapperAddress} to ${testUnwrap.address} (tx: ${hash})...`);
    ...
    const hash = await testUnwrap.write.setWrapperApproval([wrapperAddress, true]);
