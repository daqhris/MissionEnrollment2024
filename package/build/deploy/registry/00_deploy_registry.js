...
const func = async function (hre) {
  ...
  if (network.tags.legacy) {
    ...
    const setRootHash = await legacyRegistry.write.setOwner([zeroHash, owner.address], {
      gas: 1000000n,
    });
    ...
    const revertRootHash = await legacyRegistry.write.setOwner([zeroHash, zeroAddress]);
  ...
  if (!network.tags.use_root) {
    ...
    switch (rootOwner) {
      case deployer.address:
        ...
        console.log(`Setting final owner of root node on registry (tx:${hash})...`);
