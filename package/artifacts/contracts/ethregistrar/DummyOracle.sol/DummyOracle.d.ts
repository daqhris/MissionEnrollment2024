// This file was autogenerated by hardhat-viem, do not edit it.
// prettier-ignore
// tslint:disable
// eslint-disable

import type { Address } from "viem";
import type { AbiParameterToPrimitiveType, GetContractReturnType } from "@nomicfoundation/hardhat-viem/types";
import "@nomicfoundation/hardhat-viem/types";

export interface DummyOracle$Type {
  "_format": "hh-sol-artifact-1",
  "contractName": "DummyOracle",
  "sourceName": "contracts/ethregistrar/DummyOracle.sol",
  "abi": [
    {
      "inputs": [
        {
          "internalType": "int256",
          "name": "_value",
          "type": "int256"
        }
      ],
      "stateMutability": "nonpayable",
      "type": "constructor"
    },
    {
      "inputs": [],
      "name": "latestAnswer",
      "outputs": [
        {
          "internalType": "int256",
          "name": "",
          "type": "int256"
        }
      ],
      "stateMutability": "view",
      "type": "function"
    },
    {
      "inputs": [
        {
          "internalType": "int256",
          "name": "_value",
          "type": "int256"
        }
      ],
      "name": "set",
      "outputs": [],
      "stateMutability": "nonpayable",
      "type": "function"
    }
  ],
  "bytecode": "",
  "deployedBytecode": "",
  "linkReferences": {},
  "deployedLinkReferences": {}
}

declare module "@nomicfoundation/hardhat-viem/types" {
  export function deployContract(
    contractName: "DummyOracle",
    constructorArgs: [_value: AbiParameterToPrimitiveType<{"name":"_value","type":"int256"}>],
    config?: DeployContractConfig
  ): Promise<GetContractReturnType<DummyOracle$Type["abi"]>>;
  export function deployContract(
    contractName: "contracts/ethregistrar/DummyOracle.sol:DummyOracle",
    constructorArgs: [_value: AbiParameterToPrimitiveType<{"name":"_value","type":"int256"}>],
    config?: DeployContractConfig
  ): Promise<GetContractReturnType<DummyOracle$Type["abi"]>>;

  export function sendDeploymentTransaction(
    contractName: "DummyOracle",
    constructorArgs: [_value: AbiParameterToPrimitiveType<{"name":"_value","type":"int256"}>],
    config?: SendDeploymentTransactionConfig
  ): Promise<{
    contract: GetContractReturnType<DummyOracle$Type["abi"]>;
    deploymentTransaction: GetTransactionReturnType;
  }>;
  export function sendDeploymentTransaction(
    contractName: "contracts/ethregistrar/DummyOracle.sol:DummyOracle",
    constructorArgs: [_value: AbiParameterToPrimitiveType<{"name":"_value","type":"int256"}>],
    config?: SendDeploymentTransactionConfig
  ): Promise<{
    contract: GetContractReturnType<DummyOracle$Type["abi"]>;
    deploymentTransaction: GetTransactionReturnType;
  }>;

  export function getContractAt(
    contractName: "DummyOracle",
    address: Address,
    config?: GetContractAtConfig
  ): Promise<GetContractReturnType<DummyOracle$Type["abi"]>>;
  export function getContractAt(
    contractName: "contracts/ethregistrar/DummyOracle.sol:DummyOracle",
    address: Address,
    config?: GetContractAtConfig
  ): Promise<GetContractReturnType<DummyOracle$Type["abi"]>>;
}
